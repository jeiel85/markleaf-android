...
### Fixed
- **설정 → Markdown → 본문 폭에 무엇을 하는 설정인지 설명을 붙였습니다 ([#465](https://github.com/jeiel85/markleaf-android/issues/465), [#466](https://github.com/jeiel85/markleaf-android/pull/466)).** 이 설정은 본문 열이 넓어질 수 있는 최대 폭을 제한합니다(좁게·편안하게·넓게가 각각 640·800·960dp). 태블릿의 2단 레이아웃 편집기와 파일 뷰어에 적용됩니다. 세로로 든 휴대폰은 어느 옵션보다도 좁아서 휴대폰에서는 세 옵션이 똑같아 보였고, 이름만 보면 줄 간격 설정처럼 읽혔습니다. 이제 옵션 아래에 11개 언어로 짧은 설명이 표시됩니다: 넓은 화면에서 줄이 너무 길어지지 않게 하고, 세로로 든 휴대폰에서는 달라지지 않으며, 줄 간격은 바꾸지 않는다는 내용입니다. 제보해 주신 [@ray4423](https://github.com/ray4423)님께 감사드립니다.

## v2.53.1 - 원어민이 검토한 슬로바키아어 (Slovak, reviewed by a native speaker) - 2026-09-24

번역만 바뀐 업데이트입니다. 동작, 권한, 저장 형식 변경은 없습니다.

### Fixed
- **슬로바키아어 번역을 원어민이 검토하고 고쳤습니다 ([#458](https://github.com/jeiel85/markleaf-android/pull/458), [#461](https://github.com/jeiel85/markleaf-android/pull/461)).** v2.53.0에서 슬로바키아어를 기여한 [@padi-sk](https://github.com/padi-sk)님이 앱 화면과 첫 실행 노트를 검토했습니다. 글자 크기 옵션의 문법 호응, 줄 폭 설정의 이름, 여러 설정 설명의 자연스러운 표현, 개인정보 화면의 "100% 로컬" 문구 등을 고쳤습니다. 팔레트 이름은 칩과 설명 모두 *Markleaf green*으로 통일했습니다. 첫 실행 노트는 처음 한 번만 만들어지므로 이미 설치한 경우 기존 노트는 그대로이고, 나머지는 모두 바뀝니다.

## v2.53.0 - 슬로바키아어 지원 (Markleaf in Slovak) - 2026-09-23

새 언어 하나와 편집기 수정 하나입니다. 권한이나 저장 형식 변경은 없습니다.
...
이슈 대응 한 번에서 나온 두 가지 수정으로, 편집기 편의 기능 하나와 폴더 동기화를 쓰는 분들을 위한 데이터 안전성 수정입니다. 권한이나 저장 형식 변경은 없습니다.

### Added
- **여러 줄을 선택하고 체크리스트 버튼을 누르면 이제 모든 줄이 각각의 항목이 됩니다 ([#446](https://github.com/jeiel85/markleaf-android/issues/446)).** 일반 줄은 `- [ ] 내용`이 되고, 이미 체크된 줄은 토글되며, 빈 줄은 빈 항목이 되지 않고 그대로 남습니다 — 문자 메시지로 받은 장보기 목록처럼 한 줄에 하나씩 붙여 넣은 목록을 한 번에 체크리스트로 바꿀 수 있습니다. 지금까지는 선택 영역의 첫 줄만 바뀌었습니다. [@Kamul-PL](https://github.com/Kamul-PL)님이 요청했습니다.

### Fixed
- **동기화 폴더의 파일이 다른 곳에서 비워진 노트가, 열었다 닫기만 해도 영구 삭제되던 문제를 고쳤습니다 ([#443](https://github.com/jeiel85/markleaf-android/issues/443)).** 편집기를 빈 노트 상태로 나가면 그 노트를 버리는 동작은, 만들고 나서 한 글자도 쓰지 않은 노트를 위한 것입니다. 다른 기기에서 비워졌거나 동기화 클라이언트가 잘라낸 파일을 가진 기존 노트는 편집기 안에서 완전히 똑같아 보였기 때문에 함께 버려졌습니다 — 노트와 첨부파일, 동기화 폴더의 파일까지, 되돌릴 휴지통 단계도 없이. 이제 Markleaf는 버리기 전에 그 행의 마지막 확정된 쓰기가 이번 방문에서 입력한 것이 아니라 가져오기였는지를 확인합니다. 한 번도 동기화한 적 없는 노트는 영향이 없습니다. [#428](https://github.com/jeiel85/markleaf-android/issues/428)과 [#434](https://github.com/jeiel85/markleaf-android/issues/434)를 작업하다가 발견했습니다.

## v2.51.0 - 동기화된 변경이 열린 노트에 바로 (Synced changes reach the open note) - 2026-09-20

이슈 대응 한 번에서 나온 세 가지 수정으로, 모두 긴 노트와 동기화 폴더에 관한 것입니다. **저장 형식 변경이 하나 있습니다:** 동기화 폴더의 노트 파일 frontmatter에 `body_sha256` 줄이 추가됩니다 — *Changed* 항목을 보세요. 권한 변경은 없습니다.

### Added
- **열어 둔 노트가 동기화 폴더에서 들어온 새 버전을 바로 반영합니다 ([#428](https://github.com/jeiel85/markleaf-android/issues/428)).** 지금까지 편집기는 노트를 한 번만 읽어서, 보고 있는 동안 들어온 변경은 노트를 나갔다 다시 들어와야 보였습니다. 두 가지는 하지 않습니다: 저장되지 않은 입력이 화면에 있는 동안에는 절대 덮어쓰지 않고(입력한 쪽이 이기며 평소대로 저장됩니다), 노트를 *비우는* 버전은 받아들이지 않습니다. 시작 시 동기화를 요청했다가 직접 테스트해 진짜 문제를 정확히 짚어낸 [@jcesarap](https://github.com/jcesarap)님이 찾았습니다.

### Fixed
- **아주 큰 Markdown 노트를 붙여 넣어도 편집기가 멈추지 않습니다 ([#437](https://github.com/jeiel85/markleaf-android/issues/437)).** 개별 서식 조각이 1,500개를 넘거나 길이가 100,000자를 넘는 노트에서는 실시간 문법 강조가 스스로 꺼집니다. 제보자의 기기에서 200,000자 Markdown 붙여넣기는 약 30초가 걸린 뒤 "Markleaf가 응답하지 않습니다"로 끝났고, 강조를 끈 같은 붙여넣기는 즉시 끝났습니다. 일반 산문은 길이와 상관없이 강조가 유지됩니다 — 강조할 것이 없어 비용도 없기 때문입니다. [@angelblackcoat](https://github.com/angelblackcoat)님이 제보하고 네 가지 크기로 직접 측정해 주셨습니다. 아주 긴 노트의 스크롤이 여전히 느린 것은 별개 한계이며 이번에 고쳐지지 않았습니다.
- **중단된 동기화 쓰기가 더 이상 엉뚱한 "(copy from another device …)" 노트를 남기지 않습니다 ([#434](https://github.com/jeiel85/markleaf-android/issues/434)).** 느린 동기화 폴더로의 쓰기가 끝나지 못하면, 다음 동기화가 Markleaf 자신이 쓴 예전 파일을 다른 기기의 편집으로 읽고 충돌 사본을 노트 옆에 만들 수 있었습니다. 이제 자신의 미완료 쓰기와 외부의 실제 편집을 구분하므로 그 사본이 생기지 않습니다. 다른 앱이 한 편집은 지금까지와 똑같이 가져옵니다. 느린 회선에서 rclone/RSAF로 동기화하는 [@ray4423](https://github.com/ray4423)님이 제보했습니다.

### Changed
- **동기화 폴더의 노트 파일 frontmatter에 `body_sha256` 줄이 들어갑니다 ([#434](https://github.com/jeiel85/markleaf-android/issues/434)).** 노트 본문의 체크섬이며, 위 수정이 가능한 이유입니다 — 이것이 없으면 Markleaf의 미완료 쓰기와 다른 앱의 편집을 구별할 수 없습니다. 기존 파일은 해당 노트를 다음에 저장할 때 이 줄이 생기고(한꺼번에 다시 쓰지 않습니다), 줄이 없는 파일은 지금까지와 똑같이 읽습니다. 다른 도구는 모르는 frontmatter 키를 그냥 무시합니다.

## v2.50.0 - 툴바에서 노트를 연결하고 날짜를 넣습니다 (Link notes and dates from the toolbar) - 2026-09-20

서식 툴바에서 노트를 연결할 방법을 찾다가 못 찾은 [@6r4f](https://github.com/6r4f)님의 요청입니다. 권한이나 저장 형식 변경은 없습니다.

### Added
- **서식 툴바에 노트 연결과 오늘 날짜 추가 ([#424](https://github.com/jeiel85/markleaf-android/issues/424)).** `Aa` 패널에 Markdown 링크 옆으로 두 항목이 생겼습니다 — `/wiki`와 `/date`가 이미 하던 삽입을 이제 `/`를 먼저 입력하지 않고도 찾을 수 있습니다.

### Fixed
- **위젯(Single Note)이 이제 굵게·기울임·취소선·인라인 코드를 실제로 표시합니다 ([#438](https://github.com/jeiel85/markleaf-android/issues/438)) — 지금까지는 `**`, `*`, `~~`, `` ` `` 기호가 글자 그대로 보였습니다.** 제목·목록·표·이미지·코드 블록은 여전히 입력한 그대로 보입니다 — 홈 화면 위젯은 한 줄 안의 글자 스타일은 줄 수 있어도 실제 제목이나 표 레이아웃은 만들 수 없습니다. [@Violet-RM](https://github.com/Violet-RM)님이 제보했습니다.

## v2.49.0 - 고정폭 글꼴로 쓰기 (Type in a monospace font) - 2026-09-19

이제 본문에 고정폭 글꼴을 쓸 수 있어 표의 열과 들여쓰기 단계를 맞추기 쉽습니다. 권한이나 저장 형식 변경은 없습니다.

### Added
- **고정폭 글꼴 옵션 ([#432](https://github.com/jeiel85/markleaf-android/issues/432)).** 설정 → Markdown → 글꼴에서 산세리프·세리프 옆에 고정폭(Monospace)을 고를 수 있습니다. 시스템 고정폭 글꼴을 쓰므로 번들되는 글꼴은 없고, 노트뿐 아니라 편집기·미리보기·주변 화면을 포함한 앱 전체 글자에 적용됩니다. 기본값은 여전히 산세리프라 직접 고르기 전에는 아무것도 달라지지 않습니다. 코드 블록은 원래부터 고정폭이었기 때문에 이 옵션을 켜면 주변 글자와 구분되지 않게 됩니다.

## v2.48.0 - 프로젝트 하나만 골라 가져갑니다 (Take one project with you) - 2026-09-19

이제 전체 노트가 아니라 태그에 해당하는 노트만 내보낼 수 있습니다. 권한이나 저장 형식 변경은 없습니다.

### Added
- **태그별 내보내기 ([#424](https://github.com/jeiel85/markleaf-android/issues/424)).** 설정 → 데이터에서 태그와 대상 폴더를 고르면 일치하는 노트를 각각 Markdown 파일로 저장합니다. `#project`를 고르면 `#project/alpha` 같은 하위 태그도 포함합니다. 하위 폴더나 첨부 파일은 내보내지 않습니다. [@6r4f](https://github.com/6r4f)님이 요청했습니다.

### Fixed
- 잠금 공간의 개인정보 보호 경계에 맞춰 일괄 내보내기에서 잠금 노트를 제외합니다 ([#430](https://github.com/jeiel85/markleaf-android/issues/430)). 보관함의 노트는 여전히 내보낼 수 있고 휴지통 노트는 제외됩니다.

## v2.47.0 - 미리보기를 손가락으로 확대해 자세히 봅니다 (Pinch the preview in for a closer look) - 2026-09-18

화면은 보이지만 기존 글자 크기 상한으로는 편하지 않다는 저시력 사용자의 요청입니다. 권한이나 저장 형식 변경은 없습니다.

### Added
- **미리보기 손가락 확대(핀치 줌) ([#423](https://github.com/jeiel85/markleaf-android/issues/423)).** 이제 두 손가락으로 렌더링된 노트를 브라우저나 PDF 뷰어처럼 확대할 수 있습니다 — 줄바꿈을 다시 흘리지 않고 지금 보이는 그 레이아웃을 최대 5배까지 그대로 키웁니다: 제목은 본문 대비 크기를 유지하고, 표는 열 구조를 유지하며, 줄바꿈 위치도 그대로입니다. 확대한 뒤에는 손가락 하나로 이동할 수 있고, 다시 오므리면 원래 크기로 돌아옵니다. 이는 글자 크기 설정과는 별개입니다 — 글자 크기 설정은 여전히 화면 폭에 맞춰 다시 흘리는 *확대 전* 크기를 조절하고, 핀치 줌은 영구적인 크기 변경이 아니라 지금 보이는 페이지를 잠깐 자세히 보기 위한 것입니다. 확대된 상태에서도 링크·체크박스·텍스트 선택이 모두 그대로 작동합니다 — 확대가 다른 레이아웃이 아니라 같은 레이아웃 위의 시각적 변환이기 때문입니다. [@kise82](https://github.com/kise82)님이 요청했습니다.

## v2.46.2 - 파일 뷰어의 링크도 노트를 엽니다 (The file viewer's links open your notes too) - 2026-09-18

#414 후속 수정입니다: 읽기 전용 파일 뷰어에서 상대 마크다운 링크가 처리되지 않고 있었습니다. 권한이나 저장 형식 변경은 없습니다.

### Fixed
- **읽기 전용 파일 뷰어의 상대 `.md`/`.txt` 링크가 이제 브라우저 대신 그 파일이 가리키는 노트를 엽니다 ([#414](https://github.com/jeiel85/markleaf-android/issues/414)).** '파일 열기…'를 비롯해 앱 밖의 파일을 읽는 경로가 모두 이 수정의 대상입니다 — 편집기 미리보기가 이미 쓰던 것과 같은 조회(프론트매터 id 또는 활성 동기화 모드의 사이드카 항목)를 뷰어에도 연결했습니다. 일치하는 노트가 없거나 휴지통·보관함에 있거나 잠긴 노트로 옮겨진 경우에는 추측 대신 그렇게 알립니다. 두 동기화 메타데이터 모드와 실기기에서 확인했습니다. 대상 경로에 공백이 있으면 `[링크](<some note.md>)`처럼 표준 마크다운 이스케이프(`<...>`)가 필요합니다 — 이는 링크 대상 안의 이스케이프되지 않은 공백에 대한 CommonMark 자체 규칙이며 Markleaf만의 제약이 아닙니다. [@Jackson-Nickk](https://github.com/Jackson-Nickk)님이 제보했습니다.

## v2.46.1 - 찾기가 정확한 글자에 맞춰집니다 (Find lands on the right letters) - 2026-09-18

편집 중 찾기·바꾸기 수정입니다. 권한이나 저장 형식 변경은 없습니다.

### Fixed
- **일부 언어에서 편집 모드의 찾기·바꾸기가 엉뚱한 텍스트를 선택했습니다.** 편집기는 노트를 소문자로 바꾼 사본에서 검색하고 그 사본의 위치를 그대로 썼는데, 소문자로 바꾸면 텍스트가 길어질 수 있습니다 — 터키어 대문자 `İ`는 두 글자가 됩니다. 결과 앞에 그런 글자가 하나 있을 때마다 선택이 한 글자씩 밀려 바꾸기가 다른 텍스트를 고쳤고, 끝부분의 결과는 노트 밖을 가리킬 수도 있었습니다. 이제 노트 길이를 바꾸지 않는, 미리보기 찾기와 같은 비교 방식을 씁니다. v2.46.0 검토 중 발견했습니다.

## v2.46.0 - 읽는 화면에서 바로 찾기 (Find what you are reading) - 2026-09-17

노트 내 찾기를 편집 모드 밖에서도 쓸 수 있습니다. 권한이나 저장 형식 변경은 없습니다.

### Added
- **미리보기에서 노트 내 찾기 ([#417](https://github.com/jeiel85/markleaf-android/issues/417)).** 미리보기의 ⋮ 메뉴에도 노트 내 찾기가 생겼습니다. 미리보기에서는 화면에 보이는 텍스트를 기준으로 검색하므로 굵게 쓴 단어는 별표 없이 찾아지고, 링크는 주소가 아니라 표시 이름으로 찾아집니다. 모든 결과를 강조하고 현재 결과는 더 진하게 표시하며, 이전/다음 버튼은 긴 문단·표·코드 블록 아래쪽에 있는 결과까지 각 위치로 스크롤합니다. 접힌 `<details>` 섹션 안의 결과도 개수에 포함되고, 그 결과로 이동하면 섹션이 펼쳐집니다. 바꾸기는 렌더링된 텍스트가 아니라 Markdown 원문을 바꾸는 동작이라 편집 모드에만 남겼고, 편집과 미리보기를 전환하면 열려 있던 찾기는 닫힙니다. [@ray4423](https://github.com/ray4423)님의 요청입니다.
...
### Fixed
- **노트 본문이 실제로 바뀔 때만 수정 시각이 갱신됩니다 ([#410](https://github.com/jeiel85/markleaf-android/issues/410)).** 편집기를 탭하면 자동저장이 요청될 수 있지만 본문이 같으면 노트를 다시 쓰지 않습니다. 편집한 뒤 열었을 때의 본문으로 완전히 undo하면, 중간 변경이 이미 저장되었어도 원래 수정 시각이 돌아옵니다. Undo 이력은 편집기가 열린 동안만 유지되므로, 노트를 닫았다가 다시 연 후의 시각 복원에는 적용되지 않습니다. 제보: [@ray4423](https://github.com/ray4423).

## v2.43.0 - 접을 수 있는 섹션과, 스스로 정리되는 빈 노트 (Collapsible sections, and empty notes clean up after themselves) - 2026-09-15

서로 독립된 두 가지 변경이며, 둘 다 같은 분이 제보했습니다. 권한이나 저장 형식 변경은 없습니다.

### Added
- **GitHub 스타일의 접을 수 있는 `<details>`/`<summary>` 섹션이 이제 미리보기에 표시됩니다 ([#403](https://github.com/jeiel85/markleaf-android/issues/403)).** 지금까지는 조용히 무시되었지만, 이제는 탭하면 펼쳐지고 접히는 요약 행으로 나타납니다. GitHub 문서가 권장하는 빈 줄 포함 형식과, 이슈에 실린 예시처럼 빈 줄이 없는 축약 형식 둘 다 지원합니다. `<summary>`가 없는 `<details>`는 지역화된 "Details" 표기로 대체됩니다(8개 언어). 접힌 섹션도 인쇄·PDF 내보내기에서는 전체가 그대로 나옵니다 — 종이에는 탭이라는 게 없으니까요. 중첩된 섹션과 연달아 이어지는 여러 섹션 모두 동작합니다. 닫히지 않은 `<details>`는 노트를 깨뜨리는 대신 문서 끝까지 이어져 — 그 뒤 내용이 전부 그 섹션 하나에 포함되고 — 짝이 없는 닫는 태그는 텍스트로 남지 않고 그냥 사라집니다. 제보: [@stacsk](https://github.com/stacsk).

### Changed
- **아무것도 쓰여 있지 않은 노트는 남겨두지 않고 정리됩니다 ([#405](https://github.com/jeiel85/markleaf-android/issues/405)).** 에디터를 나가면 — 뒤로가기 아이콘, 시스템 뒤로가기, 다른 노트로 전환 중 무엇이든 — 한 번도 쓰인 적 없는 노트나, 썼다가 전부 지워서 다시 빈 노트가 된 경우 모두 삭제됩니다. 노트가 평범한 상태일 때만 적용되며, 빈 노트를 휴지통으로 직접 옮기거나 보관·잠금·고정한 경우는 그대로 남습니다. 제보: [@stacsk](https://github.com/stacsk).

## v2.42.0 - 사이드로드 빌드가 스스로 업데이트합니다 (The sideload build can update itself now) - 2026-09-14

이번 변경은 사이드로드 APK에만 적용됩니다. 스토어·F-Droid 빌드는 새 코드도 새 권한도 없고, 이 릴리스로 달라지는 것이 없습니다.

### Added
- **사이드로드 빌드가 앱을 떠나지 않고 업데이트를 확인·다운로드·설치할 수 있습니다 (#397).** 설정 → 앱에서 켤 수 있으며, 기본값은 꺼짐입니다. 더 새로운 사이드로드 릴리스가 있으면 노트 목록 위에 배너가 뜨고, 탭한 뒤 다운로드를 누르면 앱이 APK를 받아 릴리스와 함께 공개된 SHA-256 값과 대조하고, Android 자체의 설치 확인 화면으로 넘깁니다. Markleaf는 스스로 아무것도 설치하지 않습니다 — 그 화면과 그 위에서의 탭만이 새 버전을 설치합니다. 다운로드나 검증이 실패하면 다시 시도하거나, 이전처럼 브라우저에서 릴리스를 여는 방법으로 돌아갈 수 있습니다. 이 기능은 사이드로드 APK에만 `INTERNET`과 `REQUEST_INSTALL_PACKAGES` 두 권한을 추가합니다 — 스토어·F-Droid 빌드는 둘 다 없고, 이 기능의 코드 자체가 그 빌드들에는 존재하지 않습니다.

## v2.41.1 - 키보드가 다시 노트 바로 아래로 (The keyboard sits under the note again) - 2026-09-14

편집 중에 키보드와 노트 사이에 빈 화면이 한 줄 띠처럼 남아 있었습니다. 권한 변경이나 저장 형식 변경은 없습니다.

### Fixed
- **편집 모드에서 키보드 위에 빈 여백이 생기지 않습니다 ([#398](https://github.com/jeiel85/markleaf-android/issues/398)).** 빈 띠의 높이는 정확히 내비게이션 바 높이였고, 그것이 원인이었습니다 — 편집기가 내비게이션 바 자리를 한 번 비워 두고, 키보드 높이만큼 또 한 번 비워 두었기 때문입니다. 키보드는 이미 내비게이션 바를 덮고 올라오므로, Android의 규칙은 둘 중 **큰 쪽**을 쓰는 것이고 더하는 것이 아닙니다. 이제 본문이 키보드 바로 위에 놓이며, 입력하는 동안 커서가 키보드 위에 유지되는 동작은 그대로이고, 키보드를 내린 상태에서는 아무것도 달라지지 않습니다. 미리보기는 처음부터 영향이 없었습니다. [@stacsk](https://github.com/stacsk) 님이 제보했습니다.

## v2.41.0 - 내가 넣은 줄바꿈은 그 자리에 (The lines you typed stay where you put them) - 2026-09-12

미리보기가 손으로 끊어 놓은 줄을 다시 이어 붙였고, Material You 위젯은 테마 설정을 거꾸로 읽었습니다. 둘 다 같은 제보에서 나왔습니다. 권한 변경이나 저장 형식 변경은 없습니다.

### Changed
- **입력한 줄바꿈이 미리보기에서도 줄바꿈으로 남습니다 ([#394](https://github.com/jeiel85/markleaf-android/issues/394)).** Markdown의 규칙은 문단 안의 단일 개행을 공백 하나로 본다는 것이어서, 짧은 줄 여러 개로 쓴 노트가 미리보기를 여는 순간 하나로 이어진 문단이 되어 돌아왔습니다. 그 규칙은 나중에 조판되는 문서를 위한 것이고, 여기서는 미리보기 옆의 편집기가 입력한 그대로를 보여 주기 때문에 끊어 놓은 줄이 눈에 띄게 사라집니다. 한국어·일본어·중국어에서 특히 나쁩니다 — 글자 사이에 공백이 들어갈 자리가 아예 없어서, 이어 붙은 자리가 문장 한가운데의 빈틈으로 보였습니다. 빈 줄은 지금까지처럼 새 문단이 되고 그 넓은 간격도 그대로이며, 아웃라인의 제목도 여전히 한 줄로 나옵니다. [@ray4423](https://github.com/ray4423) 님이 제보했습니다.

### Fixed
- **Material You 위젯이 테마 설정을 거꾸로 읽지 않습니다 ([#394](https://github.com/jeiel85/markleaf-android/issues/394), [#375](https://github.com/jeiel85/markleaf-android/issues/375)의 후속).** 테마를 **라이트**로 두면 **다크**로 둘 때보다 어두운 위젯이 나왔습니다. 야간 색을 배경화면 팔레트의 반대쪽 끝에서 가져왔기 때문입니다 — 진한 색이 들어갈 자리에 옅은 강조색이 들어가면서 홈 화면에서 둘이 자리를 바꿨습니다. 라이트는 그대로이고, 다크가 이제 둘 중 어두운 쪽입니다. 설정이 말하는 그대로입니다. Markleaf Green 위젯은 영향이 없고, Material You는 여전히 Android 12 이상이 필요합니다. [@ray4423](https://github.com/ray4423) 님이 제보했습니다.

## v2.40.0 - 문법을 몰라도 표와 콜아웃 (Tables and callouts without typing the syntax) - 2026-09-11

앱이 이미 이해하고 있던 두 가지 서식이 슬래시 명령으로만 닿을 수 있었습니다. 이제 서식 패널에 있습니다. 권한 변경이나 저장 형식 변경은 없습니다.

### Added
- **서식 패널에 표와 콜아웃이 들어왔습니다 ([#390](https://github.com/jeiel85/markleaf-android/issues/390)).** 예시 노트는 표와 `> [!NOTE]` 콜아웃을 보여 주고 미리보기도 둘 다 그려 왔지만, 키보드 위 서식 패널은 코드 블록·구분선·이미지에서 멈춰 있었습니다. 그래서 둘 중 하나를 넣으려면 Markdown을 손으로 치거나, `/`를 치면 빠른 삽입 메뉴가 열린다는 사실을 알아야 했습니다. 이제 둘 다 패널의 **블록과 미디어** 그룹에 있습니다. 표는 하나의 블록으로 들어가므로 문단 중간에서 버튼을 눌러도 표로 그려지고, 텍스트를 선택한 채 **콜아웃**을 누르면 빈 콜아웃이 아니라 그 텍스트가 콜아웃이 됩니다. [@hawk1335](https://github.com/hawk1335) 님이 제보했습니다.

## v2.39.0 - 읽고 있는 화면에서 그대로 복사 (Copying out of the page you are reading) - 2026-09-11

미리보기는 무언가를 가져가려는 순간 막다른 길이었습니다. 이번 두 가지는 모두 같은 제보에서 나왔습니다. 권한 변경이나 저장 형식 변경은 없습니다.

### Added
- **미리보기 텍스트를 선택해서 복사합니다 ([#386](https://github.com/jeiel85/markleaf-android/issues/386)).** 지금까지 렌더링된 화면은 손댈 수 없어서, 문장 하나를 복사하려면 편집 모드로 돌아가 키보드까지 띄워야 했습니다. 이제 미리보기에서 텍스트를 길게 누르면 선택되고, 평소처럼 복사 도구모음을 쓸 수 있습니다. [@Bedz01](https://github.com/Bedz01) 님이 제보했습니다.
- **링크를 길게 누르면 주소가 복사됩니다 ([#386](https://github.com/jeiel85/markleaf-android/issues/386)).** 미리보기는 링크의 표시 문구만 보여 주고 실제 주소는 보여 주지 않기 때문에, 주소를 알려면 편집 모드로 돌아가 원본 Markdown을 읽는 수밖에 없었습니다. 이제 링크를 길게 누르면 주소가 클립보드에 담깁니다. 짧게 누르면 지금처럼 링크가 열리고, 링크 위에서 끌면 그대로 스크롤됩니다. [@Bedz01](https://github.com/Bedz01) 님이 제보했습니다.

## v2.38.0 - 폴더를 연결하면 읽습니다 - 2026-09-10

동기화 폴더를 고르면 이제 노트를 내보내기만 하지 않고, 그 폴더에 이미 있던 파일도 가져옵니다.

### Fixed
- **동기화 폴더를 고르면 이미 있던 파일을 가져옵니다 ([#372](https://github.com/jeiel85/markleaf-android/issues/372), [#370](https://github.com/jeiel85/markleaf-android/issues/370)에서 드러남).** 폴더 연결은 한쪽 방향만 돌았습니다. 노트를 내보내고 몇 개를 내보냈는지 알려 준 뒤, 폴더에 이미 있던 파일은 다음 동기화 — **지금 동기화**, 또는 앱을 나갔다 다시 열기 — 를 기다렸고, 그 사실은 화면 어디에도 없었습니다. 원래 노트를 모아 두던 폴더를 가리키면 보이는 결과는 "노트를 내보냈다"는 메시지와 내 파일이 없는 목록뿐이었습니다. 이제 연결 단계에서 폴더를 함께 읽고, 양쪽 방향을 모두 알려 줍니다.
- **깊게 중첩된 노트가 앱을 닫지 않습니다 ([#262](https://github.com/jeiel85/markleaf-android/issues/262)).** 미리보기로 노트를 열면 구조를 한 단계씩 따라 들어갔는데 멈추는 지점이 없었습니다. 그래서 충분히 깊게 중첩된 목록 — 사람이 손으로 쓰는 깊이를 훨씬 넘어, 생성기나 다른 앱이 만들어 낼 수 있는 종류 — 이 있는 노트는 앱의 여유 공간을 다 써서 그 자리에서 닫혔습니다. 이제 미리보기는 갈 수 있는 깊이까지 그리고 잘린 지점을 `…` 줄로 표시하며, 그보다 더 깊은 노트는 아예 안 보이는 대신 일반 텍스트로 보여 줍니다. 평범한 노트는 지금까지와 똑같이 그려집니다.
- **홈 화면 위젯이 폴더 동기화를 따라갑니다 ([#262](https://github.com/jeiel85/markleaf-android/issues/262)).** 다른 기기에서 고친 노트가 폴더 동기화로 들어와도 위젯은 그 사실을 듣지 못해서, Markleaf를 다시 열었다 나가기 전까지 옛 내용을 보여 줄 수 있었습니다. 이제 동기화가 실제로 무언가를 가져오면 두 위젯이 바로 새로 그려지고, 편집기에서 수정한 뒤에도 같은 갱신이 일어납니다 — 최근 노트 위젯이 빠뜨리던 부분입니다.

### Changed
- **파일이 든 폴더를 가져오기 전에 먼저 물어봅니다.** 폴더를 읽는다는 것은 그 파일들이 노트가 된다는 뜻이고, 기본 설정에서는 각 파일에 작은 `---` 헤더가 들어갑니다. 아직 노트가 아닌 파일이 있으면 몇 개인지 알려 주고 먼저 확인을 받으며, **노트 메타데이터** 설정을 함께 안내합니다 — 같은 화면에 있는 설정이고, 파일을 두 번 고쳐 쓰지 않고 바꿀 수 있는 마지막 순간이기 때문입니다. 새로 가져올 것이 없는 폴더는 아무것도 묻지 않습니다. **파일 옆에**를 먼저 고르면 파일은 순수 텍스트로 남고, 그 모드에서는 헤더 안내도 뜨지 않습니다.

## v2.37.5 - 위젯이 색 설정을 따릅니다 (Widgets follow your colours) - 2026-09-09

홈 화면 위젯이 항상 Markleaf Green이 아니라 설정에서 고른 색으로 그려집니다.

### 수정
- **색상 설정이 위젯까지 적용됩니다 ([#375](https://github.com/jeiel85/markleaf-android/issues/375)).** 설정 → 모양 → 색상은 앱 화면만 바꿨습니다. 두 위젯은 고정된 초록색을 그리고 있어서, Material You를 골라 둔 상태로 위젯을 추가해도 Markleaf Green으로 나왔습니다. 이제 위젯도 배경화면에서 뽑은 색 — 앱이 쓰는 것과 같은 색이라 위젯과 위젯이 여는 화면이 서로 맞습니다 — 을 쓰고, 위에 얹히는 글자색도 런처 테마가 아니라 그 배경을 따라갑니다. Markleaf Green을 고르면 지금까지와 똑같이 보입니다. Material You는 Android 12부터이며, 그 아래에서는 앱과 마찬가지로 초록색을 유지합니다. 이미 놓아 둔 위젯도 설정을 바꾸면 다시 칠해지므로 지웠다 다시 추가할 필요가 없습니다.

## v2.37.4 - 위젯이 스크롤됩니다 (The widget scrolls) - 2026-09-08

단일 노트 위젯이 위젯 크기에서 잘리지 않고 노트 전체를 홈 화면에서 보여줍니다.

### 수정
- **단일 노트 위젯이 스크롤됩니다 ([#371](https://github.com/jeiel85/markleaf-android/issues/371)).** 지금까지는 본문을 하나의 텍스트 덩어리로 그려서 위젯 높이에서 잘렸고 나머지를 볼 방법이 없었습니다. 이제 한 줄이 한 행인 스크롤 목록이라 긴 노트도 그 자리에서 읽을 수 있습니다. 빈 줄은 간격을 유지하고, 긴 줄은 잘리는 대신 줄바꿈되며, 넘겨줄 수 있는 분량을 넘긴 노트는 `…` 행으로 끝나 잘렸다는 사실이 드러납니다. 위젯을 눌러 노트를 여는 동작도 그대로이며, 행이 아닌 영역 — 본문이 빈 노트의 위젯은 전부 그 영역입니다 — 에서도 동작합니다.

## v2.37.3 - 첫 실행부터 위젯을 준비합니다 (Widgets ready on first launch) - 2026-09-07

Markleaf를 처음 열기 전에 위젯을 추가해도 노트 선택 화면을 바로 사용할 수 있습니다.

### 수정
- **위젯 선택 화면에서 첫 실행 노트를 보여줍니다 ([#262](https://github.com/jeiel85/markleaf-android/issues/262)).** 이제 첫 실행 샘플 노트는 `MainActivity`가 시작될 때뿐 아니라 단일 노트 위젯 설정 화면을 런처가 열 때도 생성됩니다. 두 시작 경로가 겹쳐도 같은 노트가 중복 삽입되지 않도록 실행을 직렬화했습니다.

## v2.37.2 - 저장과 동기화를 함께 (Sync keeps up) - 2026-09-06

외부 Markdown 파일을 노트로 저장하면 폴더 동기화가 켜진 경우 즉시 미러링합니다.

### 수정
- **노트로 저장할 때 즉시 동기화합니다 ([#366](https://github.com/jeiel85/markleaf-android/issues/366)).** 읽기 전용 파일 뷰어에서 **노트로 저장**을 누르는 순간 설정된 동기화 폴더에도 새 노트를 씁니다. 쓰기가 성공한 뒤에만 동기화된 스냅샷을 기록하므로, 제공자를 사용할 수 없으면 이후 편집 때 다시 시도할 수 있습니다.

## v2.37.1 - 시각을 그대로 보존합니다 (Timestamps that stay put) - 2026-09-06

외부 Markdown 파일은 사용자가 명시적으로 노트로 저장하기 전까지 읽기 전용입니다.

### 수정
- **외부 파일을 노트로 가져올 때 시각을 보존합니다 ([#363](https://github.com/jeiel85/markleaf-android/issues/363)).** `파일 열기…`와 공유된 Markdown 파일은 읽기 전에 제공자가 가진 원본 수정 시각을 캡처합니다. Markleaf 파일은 `created_at`과 `updated_at` frontmatter를 유지하고, 수정 시각만 제공하는 저장소는 그 시각을 두 값에 사용합니다. 열람 중 원본 파일에는 쓰지 않습니다.

## v2.37.0 - 되돌릴 수 있습니다 (Take it back) - 2026-09-05

편집기에 실행 취소가 생겼습니다. 권한 변경이나 저장 형식 변경은 없습니다.

### Added
- **편집기의 실행 취소와 다시 실행 ([#360](https://github.com/jeiel85/markleaf-android/issues/360)).** Markleaf는 쓰는 대로 저장합니다. 그래서 지금까지는 실수도 함께 저장됐습니다 — 전체 선택 후 아무 키나 누르면 노트가 사라지고, 돌아갈 곳이 없었습니다. 이제 키보드 위 줄, 서식 버튼 옆에 **실행 취소**와 **다시 실행** 버튼이 있고, 하드웨어 키보드에서는 **Ctrl+Z** / **Ctrl+Shift+Z**(또는 **Ctrl+Y**)가 동작합니다. 선택 영역 위에 입력하거나 붙여넣기, 모두 바꾸기, 전체 삭제처럼 크게 지우는 변경은 **한 단계**로 묶여서 한 번에 되돌아옵니다. 평범한 타이핑은 묶음으로 합쳐지고 줄이 바뀌는 곳에서 끊기므로, 글자 하나씩이 아니라 적당한 덩어리로 되짚어 갑니다. 되돌린 내용도 다른 편집과 똑같이 저장됩니다. 기록은 열어 둔 노트에 한하며 노트를 나가면 사라집니다 — Markleaf는 여전히 기기에 버전 기록을 남기지 않습니다. [@iamgitcat](https://github.com/iamgitcat) 님이 요청했습니다.

### Changed
- **키보드 위 줄이 실행 취소만을 위해서도 나타납니다.** **설정 → Markdown → 서식 버튼 표시**를 꺼 두면 이 줄은 텍스트를 선택했을 때만 올라왔습니다. 이제 되돌릴 것이 생기면 그때도 올라옵니다 — 그렇지 않으면 이 설정을 끈 사람에게는 실행 취소에 손으로 닿을 방법이 없기 때문입니다. 아직 아무것도 입력하지 않았다면 예전과 똑같이 나타나지 않습니다. **노트에서 찾기**를 열었을 때도 마찬가지입니다 — 서식 컨트롤은 예전처럼 내려가지만 실행 취소는 남습니다. 모두 바꾸기야말로 가장 되돌리고 싶은 변경이기 때문입니다.

## v2.36.1 - 시작 화면까지 따라옵니다 (The splash catches up) - 2026-09-02

v2.36.0의 테마 수정이 닿지 못했던 한 곳을 마저 고쳤습니다. 권한 변경이나 저장 형식 변경은 없습니다.

### Fixed
- **시작 화면이 테마 설정을 따릅니다 ([#354](https://github.com/jeiel85/markleaf-android/issues/354)).** v2.36.0에서 노트 목록과 노트 사이의 색 깜빡임은 사라졌지만, 앱을 열 때 나오는 시작 화면은 여전히 설정한 테마가 아니라 시스템 쪽 색이었습니다 — Markleaf를 다크로 두어도 밝은 휴대폰에서는 흰 화면이 지나갔습니다. 이제 첫 화면부터 설정한 테마로 나옵니다. 테마가 **시스템**이면 달라지는 것은 없습니다. Android 12 이상이며, 그 이하에서는 기존대로 시스템을 따릅니다. [@gamersat678](https://github.com/gamersat678) 님이 제보했습니다.
