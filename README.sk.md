...
- **Priama inštalácia APK**: [vydanie v2.52.0 na GitHube](https://github.com/jeiel85/markleaf-android/releases/tag/v2.52.0) obsahuje dva súbory APK — `markleaf-v2.52.0.apk` zodpovedá buildu z F-Droidu/Play (bez automatických aktualizácií, bez ďalších povolení) a `markleaf-v2.52.0-sideload.apk` pridáva voliteľnú kontrolu aktualizácií v aplikácii (`INTERNET`, `REQUEST_INSTALL_PACKAGES`). Ak chcete aktualizácie priamo v aplikácii, vyberte verziu sideload, stiahnite ju a spustite v zariadení s Androidom — obe majú rovnaký podpisový kľúč, takže neskorší prechod medzi nimi je bežná aktualizácia, nie preinštalovanie.
