...
## v2.37.6 - 깊게 중첩된 노트, 그리고 최신 상태의 위젯 (Deeply nested notes, and fresher widgets) - 2026-09-10

깊게 중첩된 목록이 있는 노트를 열어도 앱이 닫히지 않고, 홈 화면 위젯이 폴더 동기화를 따라갑니다.

### 수정
- **깊게 중첩된 노트가 앱을 닫지 않습니다.** 미리보기로 노트를 열면 구조를 한 단계씩 따라 들어갔는데 멈추는 지점이 없었습니다. 그래서 충분히 깊게 중첩된 목록 — 사람이 손으로 쓰는 깊이를 훨씬 넘어, 생성기나 다른 앱이 만들어 낼 수 있는 종류 — 이 있는 노트는 앱의 여유 공간을 다 써서 그 자리에서 닫혔습니다. 이제 미리보기는 갈 수 있는 깊이까지 그리고 잘린 지점을 `…` 줄로 표시하며, 그보다 더 깊은 노트는 아예 안 보이는 대신 일반 텍스트로 보여 줍니다. 평범한 노트는 지금까지와 똑같이 그려집니다.
- **홈 화면 위젯이 폴더 동기화를 따라갑니다.** 다른 기기에서 고친 노트가 폴더 동기화로 들어와도 위젯은 그 사실을 듣지 못해서, Markleaf를 다시 열었다 나가기 전까지 옛 내용을 보여 줄 수 있었습니다. 이제 동기화가 실제로 무언가를 가져오면 두 위젯이 바로 새로 그려지고, 편집기에서 수정한 뒤에도 같은 갱신이 일어납니다 — 최근 노트 위젯이 빠뜨리던 부분입니다.
