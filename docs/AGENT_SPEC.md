...
- `android.permission.INTERNET` 사용하지 않음 (post-MVP 범위는 §15.1·§15.9 참조 — 스토어
  배포 산출물에서는 이 금지가 계속 유효하고, 사이드로드 전용 빌드만 예외다)
- 연락처 권한 사용하지 않음
- 위치 권한 사용하지 않음
- 마이크 권한 사용하지 않음
- 카메라 권한 사용하지 않음
...
- `android.permission.INTERNET`을 추가하지 않는다. (MVP era 기준. post-MVP에서 이 제약이
  어느 산출물에 적용되는지는 §15.1·§15.9가 규정한다.)
- 독점 SDK를 추가하지 않는다.
...
- **우리 백엔드 0.** Markleaf는 자체 서버를 운영하지 않고, 노트·태그·첨부·메타데이터를 어떤
  서버로도 보내지 않는다. 이 문장은 모든 빌드에 예외 없이 적용된다.
- **스토어 배포 산출물은 INTERNET 권한 0.** F-Droid·Play로 나가는 빌드는 권한을 선언하지
  않으므로 네트워크 호출이 불가능하다. 유일한 예외는 사이드로드 전용 빌드의 업데이트
  확인이며 §15.9가 그 범위를 규정한다.
...
| §2.2 로컬 우선 | **스토어 배포 산출물**(F-Droid·Play)은 INTERNET 권한 영구 금지. sync는 SAF + 외부 도구. 예외는 §15.9의 사이드로드 업데이트 확인·설치 하나뿐이며, 노트 데이터가 기기를 떠나지 않는다는 원칙은 어떤 빌드에서도 예외가 없다. |
...
### 15.7 로컬 Quick Insert (v2.22.0)

에디터의 줄 시작에서 `/`를 입력하면 기존 Markdown 기능을 검색해 즉시 삽입한다.

- 명령 결과는 항상 표준 Markdown 텍스트이며 별도 블록 데이터나 영구 명령 문법을 저장하지 않는다.
- URL·본문 중간 슬래시는 트리거하지 않고, 현재 줄의 첫 비공백 문자가 `/`일 때만 열린다.
- 기존 포맷팅 툴바는 유지하며 터치와 외장 키보드 선택을 함께 지원한다.
- 새 권한, 네트워크, 계정, 분석, 데이터베이스 변경, 외부 의존성을 추가하지 않는다.

### 15.8 파일 열람 뷰어 (#326)

앱 밖의 `.md` / `.txt` 파일 하나를 노트로 가져오지 않고 그대로 읽는 읽기 전용 화면을 둔다.

- 진입은 두 곳이다. 노트 목록 ⋮ 메뉴의 *파일 열기…*(SAF picker)와 파일 관리자에서 온 `ACTION_VIEW`.
- 뷰어는 아무것도 쓰지 않는다. 노트는 상단바의 *노트로 저장* 을 눌렀을 때만 만들어지며, 그 경로는 기존 공유 가져오기와 같다(제목 머리말이 없으면 파일 이름이 제목).
- `ACTION_SEND`(다른 앱에서 공유)는 예전대로 곧바로 노트로 가져온다. → D069
- 파일 브라우저가 아니다. 최근 목록, 디렉터리 탐색, URI 권한 영속화, 제자리 편집을 넣지 않는다. 노트가 사는 곳은 여전히 한 군데다.
- 새 권한, 네트워크, 계정, 분석, DB 스키마 변경, 외부 의존성을 추가하지 않는다.

### 15.9 사이드로드 빌드의 업데이트 확인 (Phase 34, D073)

직접 APK를 받아 쓰는 사용자가 새 버전을 놓치지 않도록, **사이드로드 전용 빌드에만**
업데이트 확인을 넣는다. 설계는 `docs/UPDATE_STRATEGY_EVALUATION.md`, 결정은 D073이다.

이 절은 §2.2의 예외를 하나 만들지만, 그 예외의 범위를 다음으로 **좁게 한정**한다.

- **산출물 분리.** Gradle 속성 게이트로 나눈다. 속성 없이 빌드하면 스토어 배포 산출물이고,
  `-Pmarkleaf.updater=true`로 빌드해야만 사이드로드 산출물이 된다. F-Droid와 Play는 속성을
  주지 않으므로 자동으로 전자를 만든다. `android.permission.INTERNET`과 업데이터 코드는
  게이트 뒤의 소스(사이드로드 전용 매니페스트와 소스 디렉터리)에만 존재하며, 스토어 빌드의
  권한 목록은 이 절이 도입되기 전과 동일하다. (2026-09-13 D074 — productFlavor에서 전환.
  분리의 보장 내용은 그대로이고 수단만 바뀌었다.)
- **나가는 데이터 0.** 네트워크 동작은 두 가지뿐이다 — 하루 1회 정적 JSON을 읽는 GET,
  그리고 사용자가 설치를 탭했을 때만 일어나는 업데이트 파일 자체의 GET(C단계). 둘 다
  받기만 하는 요청이고, 노트·태그·첨부·메타데이터·식별자·사용 기록은 어떤 빌드에서도
  전송하지 않는다. 분석·원격 설정·크래시 리포팅 금지는 그대로 영구 유지된다.
- **기본 꺼짐.** 사용자가 설정에서 직접 켜야 동작하고, 켤 때 F-Droid의 검증을 우회하는
  경로임을 같은 화면에서 설명한다(F-Droid 포함 정책 요구).
- **F-Droid 재현 빌드 보호.** `Binaries:` + `AllowedAPKSigningKeys` 검증 대상은 스토어
  산출물이며, 그 자산은 반드시 속성 없이 빌드한 것이어야 한다. 두 빌드가 같은 출력 경로를
  쓰므로 릴리스 워크플로는 스토어 APK를 먼저 만들어 옮긴 뒤 사이드로드 APK를 빌드한다.
  순서를 뒤집으면 F-Droid가 검증할 자산이 사이드로드 빌드가 된다. 업스트림 fdroiddata
  레시피는 고치지 않는다 — F-Droid는 속성 없이 빌드하므로 `gradle: - yes`가 계속 맞다.
- **Play 정책 보호.** Play로 올라가는 산출물에는 업데이터가 들어가지 않는다. Play의 자체
  업데이트 금지 조항에 걸리는 빌드를 만들지 않는다.

확장 금지: 이 절은 업데이트 확인·안내와, 사이드로드 빌드의 APK 다운로드·설치 위임까지만
허용한다(다운로드·설치는 2026-09-14에 구현됨). 노트 동기화, 계정, 원격 백업, API 연동으로의
확장 근거로 인용할 수 없다 — 그것들은 `docs/NETWORK_FEATURE_NECESSITY_EVALUATION.md`에서
여전히 보류 상태다.
