...
- [x] 부모 태그 선택 시 하위 태그를 포함하고, 잠금·휴지통 노트는 제외
- [x] v2.48.0 릴리스 문서와 10개 스토어 로케일 갱신
...
- [x] `MarkdownPreviewList`에 `previewZoomGesture` 이음매 추가 — `PointerEventPass.Initial`에서
      ...
      같은 패턴)
- [x] 자체 검토로 발견: `scale`/`translationX`를 `pointerInput(Unit)` 캡처 파라미터로 읽어 두 번째
      핀치가 첫 번째 결과가 아닌 최초 컴포지션 값에서 다시 시작하던 버그 — `rememberUpdatedState`로
      수정, 되돌려서 실패 확인 후 복원
- [x] Codex 리뷰(PR #427) 지적 2건 모두 수정: (1) LazyColumn이 스케일 무관하게 전체 높이로
      측정돼 확대 시 스크롤 범위가 축소분을 반영 못해 하단 절반가량이 도달 불가 — 높이만
      `scale`로 나눠 측정 후 배치 시 확대하는 방식으로 교체(너비는 리플로우 방지를 위해 그대로),
      (2) 확대 상태에서 1포인터 이동이 항상 팬으로 소비돼 텍스트 선택(롱프레스 시작)이 막힘 —
      `linkPressGestures`와 같은 3방향 레이스(핀치 시작/이동/롱프레스 타임아웃)로 교체.
      두 수정 모두 되돌려서 회귀 테스트 실패 확인 후 복원
- [x] Compose `performTouchInput`으로 실제 2포인터 핀치를 주입하는 통합 테스트 7건 — adb로는 진짜
...
- [x] Codex 리뷰 지적(행이 아닌 영역의 탭 상실)을 수정하고 PR #373 머지
- [x] v2.37.4 / versionCode 139 릴리스 문서·8개 로케일·`D:\Build` 산출물 검증
...
- [x] `sourceSets.main.java.srcDir` 게이트 확인 — 등록/제외는 `:app:sourceSets`로 실측(속성 없이 `[src/main/java]`, 속성 주면 둘 다), 컴파일은 CI의 새 스텝이 `assembleDebug -Pmarkleaf.updater=true`로 돌린다. 다만 `src/sideload/java`는 아직 비어 있으므로 **소스 기여분이 실제로 컴파일되는 것은 6·7번에서 코드가 들어갈 때**다. 웹 세션 컨테이너에는 Android SDK가 없어 여기서는 컴파일을 돌릴 수 없다
