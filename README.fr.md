...
**Une seule exception.** L'APK publié sur [GitHub Releases](https://github.com/jeiel85/markleaf-android/releases/latest) déclare `INTERNET` pour une seule chose : une vérification de mise à jour **désactivée par défaut, à activer explicitement**, qui lit un unique fichier JSON statique. Les versions F-Droid et Google Play ne contiennent ni cette permission ni ce code — non pas désactivés, mais **absents**. **Aucune version n'envoie de notes, d'étiquettes, de pièces jointes, de métadonnées, d'identifiants ni de données d'usage.** Cette limite est consignée dans [`docs/AGENT_SPEC.md` §15.9](docs/AGENT_SPEC.md).
