...
- **The goldens moved exactly where they should.** Recorded on the Linux runner (`record_roborazzi`), never locally: eleven images changed — every `editor_formatting_*` golden and `editor_screen_quiet_appbar_phone` for the new pair, and `settings_centered_tablet` for the reworded "Show formatting button" description — two are new, and the remaining 62 came back byte-identical.
