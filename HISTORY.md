...
- Implementation: PR #NNN. `TrashScreenTest` (Robolectric) renders a trash row whose title is longer than the screen and asserts both action texts are displayed, that the title's right edge does not cross the Recover button's left edge, and that Recover sits left of Delete.
