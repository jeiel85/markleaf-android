...
- **11개 언어 UI** — 한국어 / 영어 / 스페인어 / 일본어 / 프랑스어 / 독일어 / 중국어 간체 / 크로아티아어 / 러시아어 / 베트남어 / 슬로바키아어 리소스 운영
...
- **APK 직접 설치**: [GitHub v2.53.2](https://github.com/jeiel85/markleaf-android/releases/tag/v2.53.2) 릴리스에는 APK가 두 개 있습니다 — `markleaf-v2.53.2.apk`는 F-Droid/Play와 같은 빌드(자동 업데이트 없음, 추가 권한 없음)이고, `markleaf-v2.53.2-sideload.apk`는 앱 내 업데이트 확인 기능이 켜져 있는 빌드입니다(`INTERNET`, `REQUEST_INSTALL_PACKAGES` 권한 추가). 앱에서 바로 업데이트를 확인하고 싶다면 사이드로드 쪽을 받아 Android 기기에서 실행해 설치하세요 — 둘은 같은 서명 키를 쓰므로, 나중에 서로 바꿔 깔아도 재설치가 아니라 평범한 업데이트로 처리됩니다.
