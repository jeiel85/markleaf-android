...
- GitHub Releases의 사이드로드 배포본은 이 권한과 `android.permission.REQUEST_INSTALL_PACKAGES`를 선언한다. 용도는 기본 꺼짐 옵트인 업데이트 확인·설치 하나뿐이다 — 하루 1회 정적 JSON GET, 그리고 사용자가 설치를 탭했을 때만 업데이트 파일 자체를 GET해 SHA-256을 대조한 뒤 `PackageInstaller`에 넘긴다(D073·D074·D075, `docs/AGENT_SPEC.md` §15.9). 노트·태그·첨부·메타데이터·식별자·사용 기록은 **어느 빌드에서도** 전송되지 않는다.
