...
### Changed
- **키보드 위 줄이 실행 취소만을 위해서도 나타납니다.** **설정 → Markdown → 서식 버튼 표시**를 꺼 두면 이 줄은 텍스트를 선택했을 때만 올라왔습니다. 이제 되돌릴 것이 생기면 그때도 올라옵니다 — 그렇지 않으면 이 설정을 끈 사람에게는 실행 취소에 손으로 닿을 방법이 없기 때문입니다. 아직 아무것도 입력하지 않았다면 예전과 똑같이 나타나지 않습니다.
