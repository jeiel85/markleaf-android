...
아직 어떤 릴리스에도 실리지 않은, `main`에 이미 들어간 변경입니다. 다음 릴리스를 자를 때 이 제목이 `## vX.Y.Z - 제목 - YYYY-MM-DD`가 되면서 함께 나갑니다. 릴리스 job은 푸시된 태그와 일치하는 제목의 섹션만 읽으므로, 이름 없는 섹션은 아무것도 배포하지 않고 아무것도 실패시키지 않습니다 — 그래서 사용자에게 보이는 변경은 릴리스 시점에 기억해내는 대신 들어오는 시점에 적어 둡니다.
