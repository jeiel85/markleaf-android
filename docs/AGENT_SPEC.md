...
| §2.2 로컬 우선 | **스토어 배포 산출물**(F-Droid·Play)은 INTERNET 권한 영구 금지. sync는 SAF + 외부 도구. 예외는 §15.9의 사이드로드 업데이트 확인 하나뿐이며, 노트 데이터가 기기를 떠나지 않는다는 원칙은 어떤 빌드에서도 예외가 없다. |
...
- **나가는 데이터 0.** 네트워크 통신은 정적 JSON 한 건을 읽는 GET 하나뿐이다. 노트·태그·
  첨부·메타데이터·식별자·사용 기록은 어떤 빌드에서도 전송하지 않는다. 분석·원격 설정·
  크래시 리포팅 금지는 그대로 영구 유지된다.
...
확장 금지: 이 절은 업데이트 확인·안내와, 그 뒤 단계인 사이드로드 빌드의 APK 다운로드·
설치 위임까지만 허용한다. 노트 동기화, 계정, 원격 백업, API 연동으로의 확장 근거로
인용할 수 없다 — 그것들은 `docs/NETWORK_FEATURE_NECESSITY_EVALUATION.md`에서 여전히
보류 상태다.
