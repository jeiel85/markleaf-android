## 2026-09-26 — Say what "Line width" does (#465)

`ray4423` read Settings → Markdown → "Line width" as CSS `line-height` and saw nothing change. Both halves checked out. The setting is a cap on the text column (`EditorLineWidth`: 640 / 800 / 960 dp), not line spacing, and Markleaf has no line-spacing setting; line height only moves with font size. The cap is applied in two places only, the Expanded-width tablet editor pane and the file viewer (`MarkleafNavHost`), and has been since it was added (7900bca6). The phone editor route never applies it, and an upright phone is narrower than all three options anyway, so on the reporter's device every choice looks the same.

The change adds `line_width_description` in all 11 locales under the options, the same pattern as `font_description`: what it caps, that an upright phone won't show a difference, and that it isn't line spacing. The wording is limited to an *upright* phone on purpose, because the file viewer does apply the cap in landscape. Whether the phone editor should honour it in landscape, and whether a line-spacing setting is wanted, stay open on #465. `settings_centered_tablet` shows this row, so its golden is re-recorded on the Linux runner.
...
The script also stops trusting `sys.boot_completed`. Measured on an API 36 emulator with `setprop ctl.restart zygote`: that property stays 1 through a framework restart, the package and `mount` services come back in 5–6 s, and the framework finishes booting only at 13–17 s — exactly the gap #454 and #458 fell into. Readiness is now ActivityManager's `mBooted=true` from `dumpsys activity processes`, which is absent while system_server is down, false while it boots, and unaffected by `-no-boot-anim`. The script waits for it before the first attempt too, so every run proves the flag exists on the CI image (API 30) instead of finding out after a restart. A system_server PID change is logged as a restart. Reproduced locally by restarting zygote the moment `Install attempt 1` printed: `Broken pipe (32)`, then `system_server restarted during the install (pid 5529 -> 9043)`, a ~10 s wait, a successful attempt 2, and a launch.
...
- **Store descriptions and READMEs state a UI-language count nobody checked.** Six store descriptions said 7 while the app had 11 until #454 fixed them by hand. Those lines are translated, so `verify-locales.ps1` now finds them by shape (a separated list of language names plus a number) instead of by wording, and requires the number to match `config/locales.tsv`. Checked against the pre-#454 files: it names both stale files and their counts.
- **`launch-smoke`'s retry was already there; its readiness check was the gap.** The #262 note proposed a retry, but the script already retried and waited for the package service. The #454 log showed the real failure: system_server restarted under the first install, the package service answered again a minute later, and attempts 2 and 3 died on `StorageManager.getVolumes()` returning null — `mount` wasn't registered yet. The probe now also requires `service check mount`, waits up to three minutes, and runs before the first attempt as well. Probe exercised on a device (ready in 0.5 s) and with `mount` mocked missing (correctly not ready).
- **The Slovak GIF was 25% larger than its siblings.** It was the ffmpeg 8.1.2 default dither. `paletteuse=dither=bayer:bayer_scale=5` gives 1.01 MB and a *higher* PSNR against the source (43.96 dB vs the default's 43.01), so the sk clip was re-encoded and the recipe, which had only ever lived in session scratchpads, is now in `docs/assets/README.md`.

## 2026-09-24 — Starter-note structure test (#262, v2.53.0 item)

`localizedStarterNotesExist` only counted notes, and the Slovak contribution (#453) showed what that misses: a translated callout type, a wikilink hard-wrapped across two lines, a multi-word tag, and a tour entry naming a title no note had. `StarterNoteStructureTest` checks all four for every language with starter notes, using the app's own parsers (`CalloutKind.parse`, `WikilinkExtractor`, `TagParser`, `TitleExtractor`) so it accepts exactly what the app accepts; code spans are skipped because `[[…]]` there is an example. The seeder's separator became `internal` so the tests split files the way the seeder does instead of repeating the literal. Checked both ways: all ten current files pass, and the contributed Slovak file (separator restored) fails all four tests, each naming the file, note and fix. `testDebugUnitTest` (934) and `lintRelease` green.

## 2026-09-23 — PR #453: Slovak, the eleventh language (v2.53.0)

`padi-sk` opened #453 with `values-sk/strings.xml` (375 keys, 1:1 with English, all four Slovak plural forms filled) and `raw-sk/starter_notes.md`. As with hr, the files were taken into a maintainer branch under the contributor's authorship and the rest was wired around them: `config/locales.tsv`, `sk-SK` store descriptions, `README.sk.md`, `docs/index.sk.html`, `docs/privacy.sk.html`, the SK entry in all 30 existing switchers (README, landing, privacy), the language counts in `AGENTS.md` / `docs/RELEASE.md` / `docs/assets/README.md` / four store descriptions / ten READMEs, and `markleaf-tablet-sk` in mp4 / gif / still, cropped from the English master with a Slovak caption band rather than re-shot.

- **The starter notes would have seeded as one note.** The contributor translated the separator to `---markleaf-poznámka---`; `StarterNotesSeeder` splits on the literal `---markleaf-note---`, so a Slovak first launch would have produced a single note holding all six. `localizedStarterNotesExist` catches that once the locale is in `locales.tsv` — but nothing checks callout types, and `[!POZNÁMKA]` (not a type the preview knows) would have rendered as a plain quote. Also fixed in the same file: a wikilink hard-wrapped across two lines (a broken link), tags with spaces (`#ochrana súkromia` is two tags), `fun` translated inside the Kotlin sample, a heading typo (`Zkradlenie`), a stray English `and`, and "neukladá" (does not *save*) where the English says does not *upload*. Paragraphs were unwrapped to match the English, because hard wraps show up as line breaks in the editor.
- **Strings.** A side-by-side review found about ten meaning errors, fixed here: `sync_conflict_copy_suffix` was an imperative ("copy!") and is written into conflict-copy titles; `expanded`/`collapsed` are screen-reader states, not "extended"/"collapse"; `line_width` meant stroke width; `theme_mode_description` had lost half its sentence; plus sort labels, the backlink panel, the quick-note widget label, the biometric prompt ("sign in" in an app with no accounts). Grammar and the Czech `Koš` → `Kôš` as well. Style-only suggestions were left alone. All of it is offered back to the contributor on #453 as proposals, since a non-native fix is not a correction.
- `privacy_verify_local_only` was `100 % s…` — a valid `% s` conversion, the same shape as vi's `% c` that failed release lint in #418. Rewritten with a non-breaking space before `%`, which is also correct Slovak typography. `UntranslatedStringTest` failed on `font_sans`/`font_serif`/`font_monospace`: Sans/Serif joined an `sk` allowlist like hr/es/fr/de, Monospace was translated as the other ten languages do.
- The GIF came out at 1.26 MB against ru/vi's ~1.0 MB with the same script; rebuilding ru with today's ffmpeg (8.1.2) gives 1.27 MB, so it is the toolchain, not the clip. Same 960x540 / 10 fps / 238 frames.
- Verification: `testDebugUnitTest` and `lintRelease` green locally, `verify-locales`, `verify-landing-versions`, `verify-release-notes`, `check-landing-overflow.mjs` (22 pages x 6 widths). On the `markleaf-phone-api36` emulator with `cmd locale set-app-locales … sk-SK`: six separate starter notes, the `[!NOTE]` callout rendering as "Poznámka", Kotlin highlighting in the sample, and Settings showing the corrected strings. The S24 was reachable but locked, so it was not used.
...
## 2026-09-18 — Issue #423: pinch-to-magnify the preview (PR #427)

`kise82` asked for pinch-to-zoom on the rendered preview, specifically *not* the four-step `fontScale` reflow that setting already drives — confirmed in the issue thread after a smaller alternative (raising that ceiling further) was offered and explicitly rejected: reflow moves line breaks, which defeats the point of a note formatted a particular way. Built as a `graphicsLayer` transform over the already-laid-out content instead — `PreviewZoom.kt` holds the pivot-preserving scale/pan math as pure functions, and `MarkdownPreviewList` gained a `previewZoomGesture` modifier that only claims an unambiguous two-finger pinch or an already-zoomed single-finger pan, checked on the same `PointerEventPass.Initial` `linkPressGestures` already uses in this file so a plain touch at rest is untouched. Caught two real defects before any of this reached a device or a reviewer: `scale`/`translationX` read as captured `pointerInput(Unit)` parameters froze at their first-composition values, so a *second* separate pinch silently restarted from scale 1 instead of building on the first — fixed with `rememberUpdatedState`, pinned by reverting the fix and watching a two-pinch test fail before restoring it. Codex review on PR #427 then found two more, both fixed in the same PR: `LazyColumn` was measured at full height regardless of scale, so its own scroll range was computed for the unscaled content and roughly the bottom half of anything shorter than the viewport was unreachable once zoomed — fixed by measuring the list at `height / scale` and magnifying the placement back up, so its scroll range accounts for how much magnified content doesn't fit on screen at once (width is left alone on purpose: shrinking it too would reflow text, the one thing this feature promised not to do); and every single-finger move while zoomed was claimed as pan immediately, which made the existing long-press text selection unreachable while zoomed despite this PR's own description claiming it kept working — fixed with a three-way race (pinch starting / movement / long-press timeout) modeled on `linkPressGestures`' own long-press race in the same file, since the built-in `awaitLongPressOrCancellation` only watches one pointer and can't see a second finger joining. Both fixes are pinned by tests that were confirmed to fail against the prior code before being restored to pass. Real multi-touch isn't reachable through adb (`input` has no pinch), so the gesture-to-transform path — including both review fixes — is verified through Compose's own multi-pointer `performTouchInput`, a real two-pointer sequence through the app's actual input pipeline. Verified by hand on the `markleaf-phone-api36` emulator that scale-1 rendering and scrolling are visually unaffected; actual on-screen pinch feel could not be checked in this environment.
---

## 2026-09-18 — Issue #414 follow-up: the file viewer's links open notes too (PR #426)

Reopened after the #414 entry below shipped: the read-only file viewer (`Open file…`, and any other way of reading a file outside the app) never wired the local-link handler note preview already had, so a relative `.md`/`.txt` link there still opened a browser. Extracted the shared resolution (frontmatter id / sidecar entry, then locked/trashed/archived handling) into `data/sync/LocalNoteLinkResolver.kt` so the editor and the viewer stop carrying two copies of it, and gave `NoteFolderMirror.noteIdForFileName` a `DocumentFile`-taking seam (`noteIdForFileNameIn`) matching the existing write/delete/rename ones — this lookup had no test coverage at all before this PR. Confirmed the reporter's "filenames with spaces break the link" is CommonMark's own destination-parsing rule (`<...>` escapes it correctly already) rather than a Markleaf defect, and pinned that with a regression test. Codex review on PR #426 found the new `onLocalLinkClick` handlers calling `navController.navigate`/`Toast` directly after a Room suspend call without marshaling back to Main — the exact #235 hazard `navigateOnMain`'s doc comment already warns about in this same file — and separately that this entry was missing; both fixed in the same PR. Verified on the `markleaf-phone-api36` emulator: frontmatter-mode folder sync, `Open file…` on a link to a note with a space in its filename via `[link](<Target Note.md>)`, landed in the target note's editor. Sidecar mode is covered by the new unit tests only, not re-verified by hand this pass.
---

## 2026-09-18 - A spike walks the sync folder's subdirectories, import only (#424, PR #425)

- Trigger: `6r4f` asked in #424 for notebooks and subfolders. Folders as a feature stay declined — #325 and #189 settled that, and `AGENT_SPEC` §2.4 makes tags the mechanism — but the request surfaced something that is not a feature request at all: **a `.md` file in a subdirectory of the linked folder is invisible to Markleaf.** Every read path lists one level, `MirrorFileLookup.isMirrorEntry` requires `isFile`, and a directory is therefore skipped with no error and no count. Point sync at a vault that already has folders and those notes never arrive, silently. This spike asks what the import direction costs to fix, and deliberately does not fix it.
- **`MirrorTraversal` holds the enumeration decision that was copy-pasted into eight places**, four of them in `MirrorWrite`. Three now route through it — the survey and both import paths, i.e. everything that reads the folder *into* notes. The write paths and `noteIdForFileName` stay flat on purpose: they decide where a file goes, and nothing yet records where one came from.
- **`maxDepth` defaults to 0 at every level and no caller passes anything else.** Depth 0 reproduces the old flat listing exactly — same files, one `listFiles()`, and the same per-entry queries — so no user's folder is read differently than before this branch. The parameter exists so a test, or a later settings-driven import, can ask for more without a second code path to keep in step.
- **The traversal was the cheap part. What blocks shipping it is that a note does not remember where it came from.** `Note` has no path field, so an imported `projects/alpha.md` is written back to `alpha.md` at the root while the original stays: one note, two files. Import-only recursion is not shippable alone; it needs `Note.relativePath`, a migration 18 → 19, and `MirrorWrite` honouring the path. Six further findings are written up in `docs/NESTED_FOLDER_SPIKE.md`, the riskiest being that sidecar mode cannot express a tree at all — `SidecarEntry` keys on a bare filename, so two `note.md` in different directories collapse onto one entry, and fixing that means bumping a schema version on a format **other devices read**, which is the road back to #140. `effectiveDepth` forces that mode flat, and the survey resolves depth through the same function so the two passes cannot disagree about which files exist (#372's lesson).
- **On `DocumentFile`, every property read is IO — and that is the whole of what review found in the code.** `isFile`, `isDirectory` and `name` each run `getRawType`/`queryForString` → `ContentResolver.query`, with no caching, confirmed by reading the `documentfile-1.0.1` bytecode rather than from memory. The first version of the walk asked `isDirectory` before the file test, costing one extra provider round trip per entry at a depth where recursion is switched off — roughly 400 on a 400-note folder, on the path `MirrorFileLookup` already records as scan-dominated (#222). Re-reading that fix turned up the same mistake again in it: `isMirrorEntry` reads `name` internally, so calling it and then building a path fetched the name twice. Both are counted by mocks now, because `RawDocumentFile` cannot show a query count.
- **A rule that was correct while depth 0 was the only depth became wrong the moment the parameter existed, twice.** The `attachments/` exclusion matched at every depth, which would have refused a user's `projects/attachments/` and every note in it — the traversal causing the exact silent loss it was written to describe. Then the same rule folding case would have refused a user's `Attachments/` on a case-sensitive provider. Both are invisible in the default configuration, since every caller passes 0. The rule is root-only and exact-name now; hidden dot-directories stay excluded at any depth, and the two answers differ on purpose, so both are pinned.
- **Seven of the fourteen review findings were the tests claiming more than they checked.** A `runCatching` that could never fire, with a test that only passed because a mock threw where no provider does. A seeded `Attachments/` that would have passed on Linux CI and failed on a macOS checkout. Assertions on `listCalls` — a figure the walk maintains itself — standing in for the round trips they claimed to guard, first for directories the walk enters, then again for the ones a rule refuses. An `atLeastOnce()` on a test whose subject was the cost of the query. A depth-0 equivalence test that asserted every read which must *not* happen and counted none of the reads that must. Each fix was verified by reinstating the defect and watching the test fail; the `listCalls` gap in particular was demonstrated — adding a listing to the skip branch fails the invocation tests and leaves the counter test green.
- **A claim in `docs/NESTED_FOLDER_SPIKE.md` contradicted a test added in the same commit.** When a nameless file became countable, the `MirrorWalk` KDoc was corrected and the spike doc was not, leaving it asserting that both counters are always 0 at depth 0. The claim lived in two places and only one moved. Worth carrying forward: this doc restates what the code documents, and every restatement can rot on its own.
- Verification: locally, `./gradlew clean test` — the all-variant task CI runs — green across debug, release and benchmark. **844 tests is the `testDebugUnitTest` figure**, 45 of them in `MirrorTraversalTest`; release and benchmark run 675 each, so 844 is one variant's count rather than the suite's total. Plus `assembleDebug`, `verifyRoborazziDebug` and `lintRelease`, all green against an SDK installed into this session. No `CHANGELOG.md` entry, because there is no user-visible change — every caller still passes depth 0.
- **Two CI failures, neither this branch's, and one of them could not be re-run.** `instrumented-tests` failed once on the first commit at `SingleNoteWidgetRenderTest.theWidgetSurfaceOpensTheNoteWhenNoRowAreDrawn`, a widget render assertion no `data/sync/` change can reach; it passed on the next commit of the same branch with nothing under `widget/` touched between them, which is the confirming re-run. Later, `build` failed at `ApplicationNightModeTest` with `MavenArtifactFetcher` → `SocketException` — Robolectric's artifact download, before any test body ran. `rerun-failed-jobs` returned **403 Resource not accessible by integration**, so the one confirming re-run could not be spent; a `./gradlew clean test` on that same commit passed instead, where the suite counted 841 — the same 841 CI had run, one of which it failed. Both failures are recorded on the PR rather than left unremarked.
- **Not verified, and the spike says so rather than implying otherwise.** SAF's real cost is unmeasured: `DocumentFile.fromFile` is a filesystem call where the user's folder is a ContentProvider query, and there is no KVM in this session, so `listCalls` exists to be counted on a device by someone who has one. The duplicate-note-id hazard (two files carrying one `markleaf_id` once a note can be copied into a subfolder) is read from the import, not reproduced against a folder. And an unreadable directory cannot be told from an empty one at all, because both `DocumentFile` implementations swallow the failure — which is the same silent drop this spike exists to describe, one level down.
- No `.agent/decisions.md` entry was cut. The spike deliberately decides nothing: it changes no behaviour, commits the project to no direction, and can be dropped without consequence. If it is pursued, the decision belongs to the change that gives `Note` a path and ships the write direction — which is the next step, not more traversal.

## 2026-09-14 - v2.41.1: the keyboard sits under the note again (#398)

- Trigger: `stacsk` reported a band of empty screen between the keyboard and the note in edit mode, with a screenshot, on Android 13 with Heliboard. Not spacing — arithmetic, and the band was exactly one navigation bar tall.
- **Window insets are distances from the window edge, so two bottom insets overlap rather than stack.** The framework says so in its own definitions: `safeDrawing` is `systemBars.union(ime).union(displayCutout)` and `union` takes the *larger* of each side, never the sum. The editor took the sum. `Scaffold` handed the navigation-bar inset down as `PaddingValues` and `imePadding()` added the whole keyboard on top of it, so the body sat `navBar + ime` from the bottom where the framework means `max(navBar, ime)`. The excess is the navigation bar — 48dp with three-button navigation, which is the band in the screenshot.
- **Neither of the two steps is wrong on its own, which is why it survived.** material3 1.3.1's `Scaffold` does not mark the inset it passes down as consumed; `onConsumedWindowInsetsChanged` only *excludes* what an ancestor already consumed, and plain `Modifier.padding` does not provide the modifier local either. So `imePadding()` correctly saw nothing consumed and correctly added the full IME inset. `consumeWindowInsets(paddingValues)` between them is the fix: `ExcludeInsets` then makes `imePadding()` add only `(ime - navBar)`, so the sum *is* the max. Dropping `imePadding()` was never an option — it is what keeps the caret above the keyboard (#136).
- **The diagnosis was settled from the pinned artifacts rather than from memory or a device.** This session had no Android SDK when the question was asked, so `compose-bom:2024.12.01` was resolved from its own POM (material3 1.3.1, foundation 1.7.6) and those source jars were downloaded and read: `ScaffoldDefaults.contentWindowInsets`, `systemBarsForVisualComponents`, the `bottom` branch of Scaffold's inner padding, `imePadding`, `UnionInsets.getBottom` and `ExcludeInsets.getBottom`. Every link in the chain is a source line, not an inference.
- **The test measures how far the body travels, not where it sits.** A position assertion would hard-code every padding inside the editor body and the formatting row and break on any unrelated spacing change; the travel when the keyboard appears owes nothing to them. Correct is `ime - navBar`, the summed defect is `ime`, and insets that never reached composition is 0 — three values far enough apart that the failure message names which happened, so a future red run says whether to fix the editor or the test. Reinstating the defect measured **280dp against the expected 232dp**, so the 48dp is a measurement rather than a reading of the source.
- Preview was never affected: that branch takes `paddingValues` without `imePadding()`, which matches the report naming edit mode, and `imePadding()` appears nowhere else in the app.
- **`instrumented-tests` failed on the release commit, and neither failure could have been the diff.** The same job had passed on the fix commit; between the two, the only app-code change is `versionCode` and `versionName`. `NavigateAfterSuspendTest` failed on the assertion #262 has recorded twice before — the third sighting, and the second on a release commit whose only change is the version numbers. The race is real and now fixed rather than merely quiet: the tap starts `createNote()`, which suspends into Room, and the navigation runs when that resumes; while Room is working there is nothing to recompose, so the tree is idle and `waitForIdle()` returns before the editor exists. `awaitEditor()` waits for the editor's Back control, which is also the premise both navigating tests had been missing. `SingleNoteWidgetRenderTest.theHostBindsTheServiceAndGetsRows` failed in the same run and was deliberately left alone — its poll already allows ten seconds and returned zero rows, which fits the late-`onDeleted`-broadcast mechanism #262 records for that class rather than a short timeout — and it passed untouched on the next run.
- `launch-smoke` failed on the next head too, at `adb install failed after retries` inside the emulator's own `PackageManagerService`, with `assembleDebug` green in the same job: the app was never started. That is the documented family the job is non-required for, and one more data point for #262's standing question of which job earns a required check.
- **Two environment limits shaped what could be checked where, and both are worth recording.** An Android SDK was installed into this session, so the unit suite (743 tests), `lintRelease`, `assembleDebug` and `assembleDebugAndroidTest` all ran locally — but there is no KVM here, so no emulator: the instrumented suite was CI's. And the release gates are PowerShell with no `pwsh` available, so `verify-release-notes.ps1` and `verify-landing-versions.ps1` were reimplemented check-for-check in Python and run against the release commit (fastlane presence and lengths, store-description limits, both CHANGELOG editions matching in order and date across 143 sections, the three landing version spots, the README release-link lines). The real scripts then passed in CI's `build`, which is what actually gates.
- Verification so far: locally, `testDebugUnitTest` (743 tests), `lintRelease`, `assembleDebug` and `assembleDebugAndroidTest` all green, and the defect reinstated to confirm the new test fails alone. On CI, the merge run's `build` and `instrumented-tests` both passed, and the tag run's four release gates — locale manifest, public version strings, release notes, release asset list — passed in its `build` job. The tag run is green end to end — `build` (the four release gates, `verifyRoborazziDebug`, `lintRelease` and the R8 release APK), `instrumented-tests`, `launch-smoke` and `release`, with `launch-smoke` passing this time rather than dying at the install step as it had on the pull request. The Release carries the two assets D072 calls for, the signed APK and its R8 mapping, and CI's own `Verify release signing certificate` step passed before publishing them.
- Not verified on a device by this session: there is no KVM here, so every device-level claim above is CI's. Nobody has yet opened the published signed build on a phone and watched the gap be gone — the strongest evidence that it is the arithmetic in the source, the measurement of the fix, and a green managed-device suite.
- **The tag could not be pushed from this session at all**, and the first attempt from the maintainer's machine failed for a different reason worth writing down. Five pushes of `refs/tags/v2.41.1` from here died at `send-pack: unexpected disconnect` with branch pushes working normally on the same remote, and no MCP tool can create a tag. On the maintainer's clone, `git fetch origin main` reached **GitLab** — `origin` is the mirror there, frozen at v2.32.4 by D068 — so `6d3e638` did not exist to tag. The tag has to be cut against the `github` remote; the D068 note about not pushing tags to GitLab is the same trap seen from the other side.

## 2026-09-12 - v2.41.0: the line breaks you typed, and a widget reading its setting backwards (#394)

*Written after the fact, from the record: the issue, PR #395, the tag run and the published Release. Where the record does not say something, this entry says so rather than filling it in.*

- Trigger: `ray4423` filed #394 at 03:08Z with a screen recording — a note written as several short lines came back as one flowing paragraph in Preview — and asked in the same report that #375 (the Colors setting) be revisited. Both halves were answered in PR #395, merged at 12:01Z, and released the same day.
- **The report's attribution was wrong, and the PR says so with the check that settles it.** #394 read the behaviour as a regression from #386. It is not: `git diff b085279 HEAD -- CommonMarkPreviewAdapter.kt` is empty across that release, and #387 touched `MarkdownPreviewList.kt` and strings only. Folding a soft line break into a space had been the behaviour since the CommonMark parser became the only preview path in v2.3.0. The request stood; the cause was older than the release it was noticed in.
- **It is CommonMark's rule, and it is the wrong rule here.** A single newline inside a paragraph is a space in a document that gets typeset afterwards. In a notes app whose editor sits beside the preview showing the raw text, the line the author broke by hand is visibly gone. `SoftLineBreak` now emits `"\n"` in `collectInlineSegments`, and only the rendered segments move: `collectText` still joins with a space, because that string is the *flat* form of a block — what the outline lists a heading by, what a link label collapses to, what a table cell holds. `PreviewLine.segments` is what the preview draws; `text` is what everything else reads it as.
- It was worst where it was least expected, which is how it was reported: between CJK characters no space belongs at all, so the folded newline arrived as a gap in the middle of a sentence rather than as ordinary word spacing.
- Three things that already behaved this way now agree with it rather than contradicting it: a trailing two-space `HardLineBreak` had always broken the line, a blank line still starts a new paragraph with the wider spacing #340 gave it, and callouts have rendered one row per source line since they were added.
- **The widget half was a colour-role mistake, not a wiring one.** Theme = **Light** painted the Material You widget *darker* than Theme = **Dark**. Day took the light scheme's `primary` (tone 40 on white) and night took the *dark* scheme's `primary` (tone 80 on tone 20) — and tone 80 is pale, so the night surface was the lighter of the two while each pair looked reasonable in the code that chose it. `primary` inverts its lightness between schemes by design: it is a role meant to be drawn *on* a surface, not to be one, and a widget is the surface. Night now takes `dynamicDarkColorScheme`'s `primaryContainer` / `onPrimaryContainer` (tone 30 on tone 90) — the filled-surface pair, darker than day, as the setting says. Day is unchanged and Markleaf Green is untouched, that layout never having been dynamic.
- **What the tests were made to pin is the part a person can see.** `WidgetPaletteTest` asserts the *ordering* — night darker than day — rather than a pair of resource ids, because the ordering is the whole of what went wrong on the home screen, and a second test holds both pairs to 4.5:1 by spelling out the WCAG luminance rather than reaching for `androidx.core`'s copy, which is only transitively on the test classpath. For the preview half, `SimpleMarkdownPreviewTest` covers five cases including the flat `text` staying space-joined, and a new golden `line_breaks_light` captures a paragraph, a bullet, a quote and a Korean pair, every row of which was a single flowing line before.
- The `paragraph_rhythm_light` fixture became one long source line in the same change. It exists to wrap, and a hand-broken line that now stays broken would have made that golden about line breaks instead of rhythm; the rendering it captures is unchanged, which is why the golden itself did not move.
- Verification, as PR #395 records it: `./gradlew assembleDebug test verifyRoborazziDebug :app:lintRelease` green on Linux against a freshly installed SDK. Before the new golden was recorded, `verifyRoborazziDebug` against the committed set reported 742 tests with a single failure — the new snapshot, which had no golden yet — and `recordRoborazziDebug` then rewrote all 75 goldens byte-identically except that file, which is what says the container's rendering matched the CI-recorded set. The tag run on `a5b647a` was green on every job — `build`, `instrumented-tests`, `launch-smoke` and `release` — and the Release published at 13:39Z carries the signed APK and its R8 mapping.
- **Not recorded, so not claimed here: whether anyone opened the published signed build on a device.** The PR describes local and CI runs only. Unlike v2.40.0, where the after-the-fact emulator pass is written down, there is nothing in the record either way for this release.
- One known defect was left alone deliberately and is worth carrying forward: a bullet whose text wraps to a second line returns to the left margin instead of hanging under the first, because there is no `TextIndent` anywhere in the renderer. A long wrapped bullet already did this; the new golden merely makes it visible on a two-line bullet. Fixing it would move every list golden, so it wants its own change.

## 2026-09-11 - v2.40.0: tables and callouts in the formatting panel (#390)

- Trigger: `hawk1335` reported that the panel above the keyboard stopped at code blocks, dividers and images, while the sample notes show tables and `> [!NOTE]` callouts and Preview has always rendered both. Typing `/` has always offered them; that is a shape you have to already know to look for, so for anyone who does not, two formats the app fully understood were reachable only by typing the Markdown by hand.
- **One construct, two doors.** The table skeleton and the callout head moved into `MarkdownEditActions` as shared constants, and `QuickInsert` now reads them rather than holding its own copies. A user who learns the shape through the slash menu and then taps the panel row must not get a different table, and two literals in two files is how that drift starts.
- **The template was never the hard part — where it goes is.** A GFM table cannot interrupt a paragraph, so a skeleton appended straight under a line of prose renders as literal pipes, which is exactly the failure a beginner reaching for this button cannot diagnose. `/table` never met it because a slash command always runs on a line holding nothing but the query; a panel tap can land mid-sentence. So the insertion point is the end of the caret's line rather than the caret itself, and `blockLeading` / `blockTrailing` compute the separating newlines from what is actually there. The same rule covers the callout, for the same reason: a blockquote glued to the line above is swallowed into that paragraph.
- **Callout has a selection case because that is how the construct is reached.** "I wrote this paragraph, make it a note" wraps the selected lines and leaves them selected; dropping an empty callout after the selection and leaving the user to retype the text would be worse than doing nothing. Wrapping something that already holds a callout is refused rather than nested, since the `> > [!NOTE]` it would produce is broken syntax a beginner cannot unpick.
- **Codex raised two P2s on #391, both reproduced and both fixed in `d8ba2b7`.** Whitespace-only indentation survived in front of the inserted block — `blockLeading` reads through trailing spaces and tabs to decide the separator, but the insertion point was the end of that line, so four spaces or a tab turned the header row or the callout head into an indented code block. And an exclusive selection endpoint at column 0 reached a line nobody selected: `0..4` in `one\ntwo` expanded to the end of `two`. The second was fixed in `selectionLineRange` itself rather than in the callout path, because the defect belongs to the helper and its other callers had it too — `indent` and `outdent` were indenting that unselected line on Tab and Shift+Tab. Re-reading the first fix adversarially turned up a third defect, new with it: a note opening with a newline, caret on that empty first line, made the line-start arithmetic land past the range end and the substring throw. Guarded, with a test.
- **The release sat merged and unpublished for five hours.** #392 merged the version bump to `main` at 08:37 and nothing else happened: the tag is what publishes, and the tag had not been cut. Worth recording because the merge looks like the end of a release and is not — `v2.40.0` was tagged on `b0b1e29` at 14:02 and the Release appeared fourteen minutes later.
- Verification: the tag run is green end to end — `build` (including `lintRelease`, `verifyRoborazziDebug` and the version/notes/locale gates), `launch-smoke`, `instrumented-tests` and `release`. The Release carries the two assets D072 calls for, the signed APK and its R8 mapping, and the APK's certificate digest was checked again locally against `0be97352…` after download rather than taken from CI's own assertion.
- **Nothing in this feature had run on a device before the tag.** The commit that closed the review records that there was still no Android SDK in that environment, so the compile and the run were CI's, and CI's `launch-smoke` installs the *debug* APK. The published signed build was therefore exercised after the fact on a `markleaf-phone-api36` emulator: install reports 2.40.0 / 143, cold launch 1.29s, no `FATAL EXCEPTION` and no ANR. Both new rows appear under **Blocks and media**; Table inserts at the end of a sample note and Preview draws a real table rather than pipes; Callout on a line of prose inserts as its own block, separated by the blank line, with the caret on the body line, and Preview renders it as a Note. That is the R8-minified, obfuscated build, which is the one thing unit tests cannot speak for.
- The `instrumented-tests` job of the `main` push run had been hanging since 08:45 — nearly six hours, against the three minutes forty-eight the same job took on the tag run — and was cancelled. It blocked nothing: `release` needs only `build`, and the tag run's own instrumented job passed on the same tree.
- Three hardening candidates filed to #262 under `## v2.40.0`, none of them blocking: Callout's refusal to nest is silent because `EditorFormattingResult` has no way to say "deliberately did nothing"; that same guard is all-or-nothing across a multi-line selection; and `TABLE_TEMPLATE` writes English column headers into notes in all eight languages, which is inherited from `/table` but newly reachable without knowing the slash menu exists.

## 2026-09-11 - v2.39.0: copying out of the preview (#386)

- Trigger: one report from `Bedz01` asking for two things at once — selectable text in Preview, and a link's address without a trip back to the editor. Both are the same complaint: the rendered view was a dead end for anything you wanted to keep.
- **The first half is one composable.** `SelectionContainer` around the preview's `LazyColumn`, and the rendered text selects like text anywhere else. The lazy-list caveat rides along: a selection spans only the composed rows, so dragging far past the viewport drops what scrolled away. Composing the whole note instead is the cost this preview exists to avoid, so that stays.
- **The second half is a three-way gesture problem, and it took three passes to get right.** Tap-to-open, long-press-to-copy and long-press-to-select all begin with the same finger. Every wrong answer was found on a device, and every one of them looked fine in the tests that existed at the time:
  - Claiming the release on the Main pass copied the address *and* opened the browser over the note — the `LinkAnnotation`'s click handling lives inside the same `Text`, so on the Main pass it sees the release first. Claiming it on the Initial pass fixed that, and a test that reverts the pass fails.
  - Consuming before the long press was certain killed scrolling: the scrolling container reads a consumed pointer as its own gesture being cancelled, so a drag that started on a link could not move the preview at all. Nothing is consumed now until the press has been held.
  - `waitForUpOrCancellation()` reads *any* consumed event as a cancellation — including the down this gesture had just consumed itself — so the gesture returned before the press could become anything and copied nothing. The wait is hand-rolled for that reason.
- **Selection is the one that cannot be called off, only restarted.** A long press on a link is a long press to the selection above it too, and no amount of consuming reaches a timeout that has already fired. So the container is rebuilt under a new epoch when an address is copied; `listState` lives outside the key, so the scroll position survives. That choice is what allows the gesture to consume nothing early, which is what keeps scrolling alive.
- **Codex found two real defects in the first push, both P2, both fixed.** The long-press threshold had been shortened to four fifths of the platform's — a leftover from an earlier attempt to beat the selection detector by deciding first — which made a press released at 400-499ms copy an address out from under a tap Android still called a tap. And a press was abandoned only when something *else* consumed the movement, which a sideways drag off a link never is: sliding clear of a link and waiting still copied the address it started on. Distance now cancels on its own. Both are pinned by tests; the slide was confirmed on a device by the absence of the platform clipboard confirmation, which appears on every copy.
- Verification: `testDebugUnitTest` and `lintRelease` green; eight tests in `MarkdownLinkLongPressTest`, and the two pre-existing preview interaction suites (table links #197, checkbox toggles #219) still pass under the new `SelectionContainer`. On a `markleaf-phone-api36` emulator, in one note: text selects with handles and a Copy toolbar, a link long press puts the address on the clipboard with no selection and no browser, a tap still opens the link, a drag from the link still scrolls, and a press that slides off the link copies nothing.
- `instrumented-tests` failed once on `SingleNoteWidgetRenderTest`, unrelated to this change, and passed on re-run of the same commit — a flake in a required check.

## 2026-09-10 - the #262 hardening pass, and the crash it turned up (PR #382, shipped in v2.38.0)

- A sweep of the hardening items #262 had accumulated across v2.34.0, v2.34.1 and v2.36.0, taken as one pass rather than a release-by-release trickle.
- The item worth the release is one the tracker had filed as a maybe: "`renderBlock` recursion has no depth bound … if it is worth bounding at all". It is. Writing the probe test produced a `StackOverflowError` at 2,000 nesting levels, which on a device is the app closing the instant a note is opened in preview. At 3,000 the fault moves *inside* commonmark — its task-list post-processor recurses over the same tree — and no ceiling in this codebase can reach that, because the parser never returns a tree to render.
- So both halves are closed where each can be. `MAX_BLOCK_DEPTH` (64) bounds this renderer's recursion and marks the cut with the `…` the single-note widget already uses; 64 is an order of magnitude past `MaxIndentDepth` (6), the level where the preview stops drawing nesting differently at all. `parse` catches the overflow the library can still raise and degrades to one body row per line, which keeps the note readable instead of ending the process. Catching an `Error` is normally wrong and the KDoc argues the exception: the stack is unwound at the catch, commonmark's `Parser` is stateless between calls, and the only value at risk is a local list.
- The two widget items were one decision, as the tracker said. `ACTION_WIDGET_NOTES_CHANGED` had been declared, exported and handled since the widget shipped and never once broadcast, while the staleness it was built for was real: the folder import is asynchronous and can finish after the `onPause` that would have covered it. The broadcast is gone — every write path that needs this is in-process, so it bought only an entry point anyone on the device could reach — and `WidgetRefresh` makes the direct call from all three import sites plus the editor's autosave, which the recent-notes widget had been missing since #351 fixed the same race for the other one.
- The rest were duplicates that had drifted: `parseHandRolled` (no callers at all, and the only producer of `PreviewLineType.EMPTY`, which kept a dead blank-line branch alive in the renderer and cost real time during the #340 diagnosis), `SidecarMigration.isMirrorFileName` (byte-identical to `MirrorFileLookup.isMirrorFile`, hand-paired with its own `isFile` check at both call sites — the shape that went missing six times in #343), and `MirrorWrite.deleteNote` / `renameToTitle`, which took only a tree `Uri` and so could not be tested at all. They gain the `DocumentFile` seam write and import already had, and six instrumented tests, including the directory guard at the delete call site where it actually prevents data loss.
- Two items closed without a code change, deliberately. The single-note widget's truncation marker already exists — #371 added it. And `PreviewLineRenderer`'s middle parameter needs no move: `isFirstBlock` is the only `Boolean` in the signature, so a positional caller gets a compile error rather than the silent breakage the item feared; moving it now would re-create the hazard for code written since.
- DESIGN.md now records the preview's 2dp sub-grid as the deliberate exception it is instead of leaving §4's 4dp rule quietly untrue, and AGENTS.md records the two checks that catch a "pure code motion" change that moved more than code — the pair that found the six dropped guards in #343 after a comment-frequency diff had passed them.
- Codex raised one finding, that the PR carried no CHANGELOG or HISTORY entry. Answered rather than fixed in place: this project writes both at the release, not in the feature PR, because the release job cuts the release body out of the `## vX.Y.Z` section and an entry cannot name a version that has not been chosen. Both are in this commit.
- Verification: `./gradlew test` green across all variants; CI's `build` and `instrumented-tests` both passed, the latter running the six new mirror tests on the managed API 30 device. `launch-smoke` failed on `StorageManager.getVolumes()` returning null inside the runner's own `PackageManagerService` — the APK never installed — which is the documented runner flake the job is non-required for; instrumented-tests installed and ran the same APK on the same commit.
- Not verified locally: this machine has no attached device and no installed system image, so the instrumented suite was run by CI rather than here.
- It shipped as part of v2.38.0 rather than under a number of its own. This work was prepared as v2.37.6 / versionCode 141, and #381 merged and tagged v2.38.0 — with 141 already claimed for it and this merge already in its history — while that release PR was in review. The code was therefore released before its notes existed; what this commit adds is the record, folded into the v2.38.0 section where it belongs. Nothing was re-tagged: two sessions cutting releases from one repository is what produced the collision, and re-cutting would only move it.

## 2026-09-10 - v2.38.0: linking a sync folder reads it (#372)

- @canllaith asked in #370 whether Markleaf could edit a folder of `.md` files they already had. It could — but the folder-picker flow only ran the export direction, so the answer looked like no: the screen reported how many notes were written out and said nothing about the 400 files sitting in the folder. Filed as #372, which is what PR #381 fixes.
- Both screens offering the picker (Settings, Sync Center) carried their own copy of "take the grant, write the notes out". That duplication is why the import was missing from both: it had been added to the three passes that run later — resume, **Sync now**, the Sync Center — and to neither copy of the link. `SyncFolderLink` now holds the link once and `rememberSyncFolderLinker` holds the picker, the dialog and the toast, so the two screens cannot drift again.
- Reading a folder is also the moment its files become notes, and in frontmatter mode a header is written into each one. `MirrorSurvey` counts what an import would adopt without adopting anything, and the count is what the dialog asks with; the frontmatter warning is suppressed in sidecar mode, where it would not be true. The survey mirrors the import's own matching rules, including #148 — a file belonging to an archived or trashed note is not a new note — so the number shown is the number that arrives.
- The Codex review caught a real ordering defect before merge. The first pass seeded before importing, and seeding writes a note into any unclaimed file bearing its title (the #213 adoption rule). A hand-dropped `Groceries.md`, or a file edited on another device, would have been overwritten by the local note and the import that followed would have read back only what had just been written — the user's text gone with nothing to show it existed. The order is now import first, seed second, which cannot lose anything, and the reason is written into the function rather than left to be rediscovered.
- Verification: 19 instrumented cases on `markleaf-phone-api36` (API 36), including five for the survey — the count, a file its note already owns, hidden notes, an empty folder, and that the survey changes nothing — plus one pinning the import-before-seed order. End-to-end on the same emulator: a folder of three hand-written files, the dialog naming three, and after confirming, three new notes in the list with the files still holding their own text. `./gradlew :app:testDebugUnitTest :app:lintRelease` passed.
- Release preparation updates versionCode 141, versionName 2.38.0, the eight store changelogs, and all public version surfaces.
...
- The review round caught three real defects in the first pass, all fixed before merge: the widget read night mode from its `Configuration`, which #354's `UiModeManager` override reaches at a moment this code does not control — so Theme = Dark on a light phone painted a light widget over a dark app; `onPause` refreshed the recent-notes widget with the bare `notifyAppWidgetViewDataChanged`, which reloads the rows that read the palette while leaving the container that carries the background, so the two could drift into light text on a light ground; and the mirror's `commit()` ran on the main thread. Theme now travels with Colors through the mirror and decides the palette's end directly, `onPause` goes through `refreshAll`, and the write moved to `Dispatchers.IO`.
- A second review round (Codex) found two more, both fixed before merge. The mirror started empty, so an existing Material You user upgrading — or anyone adding a widget from the launcher's picker, which starts the configure activity directly, the same route that emptied the note picker in #262 — would have kept seeing green widgets until `MainActivity` next ran, which is the reported defect surviving its own fix. And a device following the system between day and night changed neither stored setting, so nothing repainted; `updatePeriodMillis` is 0, so nothing else would have either. The mirror is now self-healing from the factories' background thread — the one place in the widget stack that may read DataStore — and the widget hands the launcher *both* surfaces through `setColorStateList`/`setColorInt`'s two-value overloads, so the host picks by its own night mode when Theme is SYSTEM and is given no choice when it is Light or Dark. Still not covered: a wallpaper accent changed while the app is closed is a snapshot until something updates the widget.
- Verification: `WidgetPaletteTest` (12 tests) covers the mirror, its self-healing, the rule, the API gate, the pair handed to the host, and the end-to-end path through `updateAppWidget` into the inflated view; each of the three defects was reinstated and failed exactly the test written for it. `./gradlew :app:testDebugUnitTest :app:lintRelease` passed.
...
- **Autosave is what makes it unrecoverable, and that framed the whole design.** Markleaf debounces a save one second after the last edit and mirrors it into the sync folder, so the wiped version reaches the row and the file before the user can react. The missing thing is a way back from the edit you just made — which is not the same feature as version history. Recorded as **D070**: in memory, per open note, dropped when the screen leaves. An on-disk store would mean storage growth, a retention policy, a browsing UI, and a second copy of every note's past sitting beside the one file per note the user owns.
- **The stack watches one value instead of being pushed to from fifteen sites.** `EditorScreen` writes `editorState` from fifteen places — typing, every formatting action, replace-one and replace-all, quick insert, wikilink and tag completion, the image insert and its alt text, indent/outdent, the preview checkbox. Pushing a step from each would be fifteen chances to forget, and the sixteenth path added later would be silently non-undoable. The screen instead feeds every `TextFieldValue` the field takes to `EditorUndoHistory.record`, which decides what is a step; values carrying no text change are ignored, so the value a restore feeds back does not become a step of its own.
- **What counts as one step took two review rounds to get right, and both findings were real.** The first rule only asked whether a diff was short and newline-free. Codex pointed out that typing `abc`, selecting it and typing `x` merged into the same run — undo then returned the empty baseline instead of `abc`. Small scale, the reported failure exactly. A change now carries its kind: `REPLACE` never joins a run however small, a run continues only in the kind it started in and at the caret the previous state left, and a caret move closes it. The second round found the other half: an insertion-only action such as Heading inserting `"# "` is *indistinguishable from two keystrokes* in the value stream, so the keystroke after it merged in. Only the app knows the difference, so `beginNewStep()` marks the next change as standing alone on both sides and every app-made edit goes through one `applyEdit` helper that sets it. Forgetting the marker degrades to a merge rather than a lost step — which is why it stays a hint rather than becoming the recording mechanism.
- **A short clipboard paste can still merge with adjacent typing, and that is stated rather than papered over.** Compose's `TextFieldValue` stream carries no paste signal; an eight-character paste and an IME committing eight characters are the same event. If a paste callback appears it drops straight into `applyEdit`.
- **Placement: the row above the keyboard, not the top bar.** In edit mode the bar already carries back, the view toggle, the outline, information and overflow — 336dp of a 360dp phone before the title. A second strip above the keyboard would have cost another 48dp. Undo and redo went into the existing formatting row instead, at its left, staying mounted and disabling rather than moving under the thumb. Six controls fit at 360dp with the selection actions showing; the link shortcut's width threshold moved from 192dp to 288dp to keep "More options" on screen below that.
- **Two mount rules had to bend, both for the same reason: the row is the only touch path to undo.** With "Show formatting button" off (#331) it used to mount only while text was selected; it now also mounts once there is a step to go back to, and stays away entirely until then. And the find bar used to take the row away wholesale — but a replace-all is the change most worth taking back. `isFormattingPreempted` was doing two jobs and is now split: the formatting controls still stand down while find is open (the selection there is the current match, not something the user chose), and undo stays.
- **Ctrl+Z started on the wrong node.** It was bound to the note `BasicTextField`, so it did nothing while the find or replace field held focus — which is exactly where focus sits after Replace All. Codex caught it; the handler moved to the editor body `Column` above all three, on the preview pass. `Ctrl+Shift+Z` and `Ctrl+Y` both redo, and Meta is accepted alongside Ctrl, matching the formatting keymap.
- **Verified on the emulator rather than reasoned about.** `markleaf-phone-api36`: opened a starter note, Ctrl+A and typed `x`, watched the row's undo light up and the redo stay dim, waited past the debounce, tapped undo — the note came back whole and the notes list showed the restored excerpt, not `x`. Then typed at the caret and pressed Ctrl+Z from the hardware keyboard, which took the insertion back. The debug build was uninstalled afterwards.
- **The goldens moved exactly where they should.** Recorded on the Linux runner (`record_roborazzi`), never locally. Thirteen of the set are different: **eleven existing images changed** — the nine `editor_formatting_*` goldens and `editor_screen_quiet_appbar_phone` for the new pair of buttons, plus `settings_centered_tablet` for the reworded "Show formatting button" description — and **two are new**, `editor_formatting_undo_available_phone` and `editor_formatting_undo_only_phone`. The set went from 73 images to 75, and the other 62 came back byte-identical.
- **The instrumented tests were checked by report, not by the job's colour.** `EditorScreenTest` ran 16 cases with 0 failures on `ciAtdApi30`, and all four new `editorScreen_undo*` methods are named in it: the select-all restore including the saved row, the formatting-action boundary, the row with the formatting button off, and the row with the find bar open.
- **One Codex P1 was rebutted rather than actioned**, the same one as in v2.36.0: it asked for `HISTORY.md` in the feature PR. `CHANGELOG.md` and `CHANGELOG.ko.md` are in it, because #361 was also the release PR; this entry is the follow-up, matching #349 and #353.
- Verification: `./gradlew test` (all variants) and `lintRelease` green locally; `verify-locales.ps1`, `verify-landing-versions.ps1`, `verify-release-notes.ps1` and `verify-release-export.ps1` green for v2.37.0 / vc135; PR CI green on `build`, `instrumented-tests`, `launch-smoke` and the landing check. Strings translated into all seven other locales, and the landing release date fixed in all eight files by hand — seven of them label it in their own language, which a version-string replace does not reach.
- The reporter was answered on the thread in English before the code was written, and again when it shipped.

## 2026-09-02 - v2.36.1: the splash the theme fix could not reach (#354)

- Trigger: the item left open on #354 when v2.36.0 shipped. The window repaint in #355 removed the flash between the notes list and a note, but a cold start with the Theme set against the system still opened on the system's colour — measured at 225.7/255 for a Dark app on a light phone, against 44 once the app drew.
- **The write-up on #354 was wrong about what was possible, and that is the useful part of this entry.** It concluded that the starting window could only be fixed by giving the splash a theme-independent background, and framed that as a visual identity decision for the maintainer. The reasoning behind it — the system creates that window before any app code runs, so no in-app override reaches it — is true and still is. What it missed is that the app does not have to reach the window; it can tell the system what its night mode *is*, in advance. `UiModeManager.setApplicationNightMode` (API 31+) persists the `-night` qualifier for the application, so the next cold start's splash is resolved the way the Theme setting asks. No brand-colour decision was needed at all.
- **Four combinations measured on `markleaf-phone-api36`, cold-start splash luma:** system light + app Dark went 225.7 → **66.2**; system dark + app Light → **229.6**; and both System cases were unchanged (229.6 on light, 66.2 on dark). Nobody whose Theme already agrees with their phone sees any difference, which is what a fixed splash colour could not have offered — it would have changed the first impression for the majority to fix a minority's flash.
- **The System mapping had to be established on a device rather than read off the API.** There is no `getApplicationNightMode`, and `MODE_NIGHT_AUTO` is documented in terms of "the device's current location and certain other sensors", not as "release the app-local override" — so whether choosing System after Dark hands control back was genuinely open, and a wrong answer would have stranded users in a mode they had turned off. It does release it: after Dark on a light phone, switching to System returned the splash to 229.6 on a light system and 66.2 on a dark one. `ApplicationNightModeTest` pins that value with the reasoning attached, because it is the one a future reader is most likely to talk themselves out of.
- **Codex raised two findings; one was real and one named a constant that does not exist.** The real one: the call was skipped when the app's effective `uiMode` already matched, but that says what the app is *rendering*, not whether an override has been *persisted* — picking Dark on an already-dark phone stored nothing, so the next system flip resolved the splash from the phone again. Reproduced exactly as described (system dark → Theme Dark → system light → cold start came back light), then made unconditional; the same scenario now gives 66.6 settling to 44.7. The other suggested `MODE_NIGHT_UNSPECIFIED`, which belongs to `AppCompatDelegate`; `android.app.UiModeManager`'s `@NightMode` IntDef is exactly `{AUTO, CUSTOM, NO, YES}`. Rebutted with that, but its second half — that `runCatching` was hiding a failure — was fair and the wrapper is gone.
- **Two claims made publicly on the strength of the wrong conclusion were corrected**: the v2.36.0 ship comment on #345 told the reporter that nothing the app does can reach that window, and the first item of the `## v2.36.0` section on #262 said the same. Both now point at this release.
- Verification: `testDebugUnitTest` and `lintDebug` green; the four splash combinations and the Codex regression scenario measured on the emulator rather than reasoned about; a cold start in each mode logs one `Displayed … MainActivity` and no relaunch of this app, checked in logcat because the call is now unconditional.

## 2026-09-02 - v2.36.0: the single-note widget, and a theme that stopped at the window (#351, #354)

- Trigger: two threads left open on GitHub. #351 asked for a widget showing a note's body; the reporter confirmed on the thread that they meant a *new* single-note widget rather than a longer preview in the existing one, and asked for a widget text size while they were there. #345 — the v2.35.0 theme-mode request — had an unanswered follow-up from `gamersat678` reporting a white flash between the notes list and a note.
- **The flash was reproduced and measured before anything was written.** `markleaf-phone-api36`, average frame luma via `ffmpeg signalstats` (0–255) across the list → editor transition: system light + app **Dark** ran 44 at rest and **189** mid-transition; system dark + app **Light** ran 226 and **86**; matched (app on System) stayed 44–55 with no excursion. The matched control is what makes it the mismatch rather than the animation. The reporter had described the reverse direction as flashing white too; it flashes dark grey — the same defect mirrored — and that correction went back on the thread.
- **The cause was one gap, not the animation.** `Theme.Markleaf` is declared twice — Material Light in `values/`, Material Dark in `values-night/` — so the platform picks between them by the `-night` qualifier, which is the *system* dark-mode setting. `AppSettings.themeMode` reached `MarkleafTheme` and nothing else; the existing `SideEffect` synced the system-bar icons but not the window background. The `NavHost` cross-fade makes both destinations translucent, which is where it showed. `MarkleafTheme` now repaints the window with `colorScheme.background` in that same effect: measured at 44.6–46.2 across the same transition afterwards.
- **The starting window is out of reach and was left open deliberately.** A cold start with a mismatched Theme still shows the platform's splash background — measured at 225.7 for a Dark app on a light system, unchanged by the fix. The system creates that window from the app's theme *before any app code runs*, so no in-app override reaches it; the fix would be a theme-independent splash background, which is a visual identity decision rather than a defect fix. Recorded on #354 with the measurement rather than shipped.
- **The single-note widget is a second widget, not a change to the first.** The recent-notes widget lists ten rows of title plus a stored 100-character summary; both can be placed and neither affects the other. A configure activity picks the note and the text size at placement; `SingleNoteWidgetStore` keeps them per `appWidgetId` in SharedPreferences, because `onUpdate` runs on the receiver's main thread and DataStore would mean blocking there. `onDeleted` drops the entry so a reused launcher id does not inherit someone else's note.
- **Text size is per widget and reuses `EditorFontSize`.** Its four steps and their existing strings, scaled around a 14sp widget body rather than the editor's 16sp — no new enum, no new vocabulary, and the editor's own setting is untouched.
- **Locked notes are kept out on both sides.** The picker reads `observeNotes`, which already filters `locked = 0`; `showableBody` is the guard for a note moved into Locked *after* it was chosen, because a home screen is visible without unlocking anything. Trashed and deleted notes take the same path and the widget says so rather than going blank.
- **Both receivers gained `android:label`.** Without it they fall back to the app name and the picker offered "Markleaf" twice; `quick_note_widget_label` had existed unused since v2.16.0, and the second widget is what made the pair ambiguous.
- **Launcher3 will not accept a synthetic drag, so the widget was verified another way.** `input draganddrop` and a hand-built `motionevent` DOWN → hold → MOVE → UP sequence both closed the widget sheet without placing anything, at several coordinates and timings. `SingleNoteWidgetRenderTest` binds a real widget id through `AppWidgetService` and calls `host.createView`, which inflates the same tree a launcher would, so the assertions are on the actual `TextView` — its text, its size in sp, and that a note locked afterwards stops showing it.
- **That test was green and inert for one commit.** `bindAppWidgetIdIfAllowed` needs `BIND_APPWIDGET`; it had been granted by hand on the local emulator, and the `assumeTrue` stood the tests down everywhere else — the CI log for the first fix commit shows both cases as `SKIPPED`. Granting from the workflow is not available because the Gradle managed device is created and destroyed inside the task, so the test now grants itself through `uiAutomation.executeShellCommand`. Verified by revoking the permission locally and rerunning (`skipped="0"`), and by reading the CI log rather than the check's colour.
- **Codex raised four findings on the widget PR and all four were real.** App lock did not cover the configure activity, which the launcher starts directly — with lock on, placing a widget listed every note's title and excerpt without authentication and could publish a body to the home screen; it is now a `FragmentActivity` behind the same `BiometricLockGate` `MainActivity` uses. A 2,000,000-character imported note handed to `setTextViewText` would throw `TransactionTooLargeException`, so the body is cut at 20,000. Pressing Home inside the editor's one-second autosave debounce ran the `onPause` widget refresh *before* the save landed, so the refresh now also runs after `autosave` persists. And four `FilterChip`s in a fixed `Row` push the last one off a narrow screen once localized — `FlowRow` instead, verified in Spanish at an overridden density of 560 (308dp), where "Extra grande" wraps and stays tappable.
- **`AppLockGateTest` took two attempts to be worth having.** Written as a plain substring scan it passed with the gate call deleted, because the leftover `import …BiometricLockGate` still carried the name. It now drops import lines and requires the call, verified by removing the gate and rerunning. The lesson is the negative control, not the test: the first version would have shipped as a green guard over nothing, exactly like the skipped instrumented tests above.
- **The one Codex finding not acted on** was a P1 asking for `CHANGELOG`/`HISTORY` in the theme-fix PR. Valid requirement, wrong PR: `CHANGELOG.md` sections are cut per version and the project puts them in the release PR — #347 (feature) carried neither, #348 (release) carried both CHANGELOGs, #349 carried HISTORY, and #353 merged the same way. Rebutted on the thread with that evidence and deferred here.
- Verification: `testDebugUnitTest` green, `lintDebug` green, and `verify-locales.ps1` / `verify-landing-versions.ps1` / `verify-release-notes.ps1` green for v2.36.0 / vc133. Both PRs' CI green including `instrumented-tests` and `launch-smoke`. Strings translated into all seven other locales; the landing release date needed fixing in all eight files by hand because seven of them label it in their own language and a version-string replace does not reach it.
- Both reporters were answered on their own threads, in English, before the code was written rather than after.

## 2026-08-31 - v2.35.0: theme mode and text size (#345, #346)

- Trigger: two feature requests from the same external reporter, `gamersat678` — a dark-theme toggle (#345) and a text-size setting (#346). Both are appearance settings, so together they are a minor.
- **Theme mode (System / Light / Dark) drives `MarkleafTheme`'s `darkTheme`.** `AppSettings` gained a `ThemeMode` enum and `AppSettingsRepository` a `THEME_MODE` DataStore key; `MainActivity` reads `isSystemInDarkTheme()` and maps the mode to `darkTheme`. The default is `SYSTEM`, so existing behaviour is unchanged. Settings → Appearance → Theme shows the three options.
- **Text size (Small / Medium / Large / Extra Large) scales the editor and the preview.** `EditorFontSize` carries a `scale` (0.875–1.25); the editor body text and the preview's headings, body, and footnote refs are scaled by it. The preview scales via a `PreviewLine.fontScale` field rather than `LocalTypography` (internal in material3), so only body text, headings, and footnote refs grow — callout labels, code blocks, frontmatter, and table cells are intentionally untouched. Checkbox glyphs and link tap targets ride on the text, so they grow with it. The default is `MEDIUM` (scale 1f), so rendering is byte-identical unless the user asks.
- **The preview's scale pipeline threads through the syntax highlighter and visual transformation.** `MarkdownSyntaxHighlighter.highlight` and `MarkdownSyntaxVisualTransformation` take a `fontScale` that scales heading metrics; `MarkdownPreviewList` stamps `fontScale` onto each `PreviewLine` and `scaledBy` multiplies font size and line height. The `scaledBy` extension is private to the preview file and only invoked at non-1f scales.
- **The branch was rebased onto a main that had moved on.** The feature branch started from v2.32.4, but main had advanced to v2.34.1 (75 commits) while the work was in flight — including a preview vertical-rhythm change (#340) that touched the same `MarkdownPreviewList.kt` and `SimpleMarkdownPreview.kt`. The rebase merged `depth`/`looseList`/`itemsIndexed`/`headingTop`/`verticalPadding` from main with the new `fontScale`/`scaledBy` cleanly. main had also gained the Croatian locale, so the new strings had to be added to `values-hr` too or `ResourceParityTest` failed.
- **The `UntranslatedStringTest` allowlist gained one entry.** `theme_mode_system` = "System" is genuinely identical in German, so it joined the `de` language-coincidence set with a reason.
- **The settings-tablet golden was re-recorded on the Linux CI runner.** The Appearance section now shows the theme-mode row, so `settings_centered_tablet.png` changed; `record_roborazzi` produced the new image and it was committed. Only that one golden changed — the other 69 were byte-identical.
- Verification: `testDebugUnitTest` green (622 tests), `lintRelease` green, `verify-landing-versions.ps1` / `verify-release-notes.ps1` / `verify-locales.ps1` green for v2.35.0, and `verify-release-export.ps1` green against the real v2.35.0 artifacts in `D:\Build` (AAB 5.6 MB, mapping 43.4 MB, 8-locale release notes). PR #347 (feature) and #348 (release) both merged; the `v2.35.0` tag pushed to GitHub only (GitLab mirror is off, D068), and the GitHub Release carries the single APK as required.
- Both issues were closed with thank-you comments in the reporter's language (English).

## 2026-08-28 - The two biggest files split along seams they already had

- Trigger: a project review flagged `NoteFolderMirror.kt` (1,232 lines) and `EditorScreen.kt` (1,651) as the two files every future change has to fight. Reading them showed the seams already existed — the mirror's decision logic was already extracted as pure `internal` functions, and the editor's tail was already pure helpers — so the split is code motion, not redesign.
- **`NoteFolderMirror` became five files behind an unchanged facade.** The object keeps its name, its nested types (`ImportResult`, `Reconcile`, `HeadScan`) and every signature, so all `NoteFolderMirror.x` references — including the seven test files that exercise the internal members — compile untouched. The implementation moved to `MirrorWrite` (DB → folder), `MirrorImport` (folder → DB), `MirrorFileLookup` (where a note's file lives: the head peek, the document cache, the name rules) and `MirrorReconcile` (the pure win/lose decisions), plus `MirrorMetadata` for the mode type and its two `AppSettings` extensions. The facade costs ~200 lines of one-line delegates; that is the price of not churning forty call sites in a motion-only PR.
- **The KDoc travelled with the code, and that was verified mechanically.** A word-frequency diff of every comment line old-vs-new across the six files shows zero words lost; the only additions are the facade's pointers and renamed `[MirrorWrite.writeNote]`-style links. The first pass of the split did lose seven fragments (a const's ceiling doc, the conflict-rule paragraph, two inline comments) — the diff caught all seven before commit, which is the argument for diffing comments rather than trusting the move.
- **`EditorScreen` dropped 515 lines of code that was only living there.** The pure text helpers (wikilink/tag autocomplete, find/replace ranges, the position-restore clamps, stats) moved to `EditorTextActions.kt`; `FindBar` to `EditorFindBar.kt`; the two suggestion rows to `EditorSuggestions.kt`; and the two inline dialogs became `ImageAltTextDialog`/`DeleteConfirmDialog` in `EditorDialogs.kt`, taking callbacks instead of capturing screen state. The composables moved; the trees did not change, so rendering is untouched by construction.
- The i18n gate caught the move, as designed: `HardcodedStringTest`'s allowlist is keyed by file, so the `"#$tag"` entry now points at `EditorSuggestions.kt`. That is the entire test diff.
- What was deliberately *not* done: no `EditorViewModel`, no state-holder extraction. The screen's `remember(noteId)` web and effect ordering encode timing contracts (#142, #200, #204, #214) that goldens cannot see; that rewrite, if ever, belongs to whichever phase rewrites the editor anyway.
- **The motion did change behavior once, and the comment diff could not see it.** Six folder listings lost their `it.isFile &&` guard along the way, so a *directory* named `Plan.md` — which sync clients do leave behind — would have counted as a note file: read by the import pass, and in sidecar mode, where the index matches by name alone, handed to `DocumentFile.delete()`, which takes a directory's children with it. Codex flagged one of the six on the PR; a normalized line-level diff of the old file against the five new ones found the other five and showed the rest of the motion was clean. The guard now sits beside the name rule as `MirrorFileLookup.isMirrorEntry`, where the two checks cannot be separated again, and `NoteFolderMirrorFolderTest` pins the import case. The lesson pairs with the one above: diffing comments proves the prose travelled, not that the code did.
- Verification: `testDebugUnitTest` green (617 tests, the mirror conflict-logic and sidecar suites now running against the new objects), `lintRelease` green. Roborazzi goldens are expected byte-identical — code motion only — and the Linux CI verify on the PR is the check of that.
...
- **The list is now `config/locales.tsv`, and the copies read it** (PR #335): both resource tests through a `LocaleManifest` helper, `noteLocales` in the release export, the three verify scripts through `scripts/locales.ps1`, and the release-notes assertion in `.gitlab-ci.yml` through one `awk` line. A four-column table rather than JSON because the readers are Kotlin, Gradle, PowerShell and a shell one-liner, and none of them should need a parser dependency for eight rows.
- **A single list only closes half of it.** Declared-but-absent is as silent as present-but-undeclared, so `scripts/verify-locales.ps1` walks both directions across app resources, starter notes, fastlane directories, landing pages, privacy pages, READMEs and the demo clips. Privacy pages are what that second direction was worth: they carry no version string, so `verify-landing-versions.ps1` never looked at them, and a language added without its translation gave two dead links per page.
- Three smaller items fell out of the same change. `.gitlab-ci.yml` had been asserting six locale blocks — missing `zh-CN` since #294 and `hr-HR` since #329 — behind a switch nobody turns on, which is exactly why nobody noticed; the job is the documented fallback for GitHub being unavailable, so its list is derived rather than the job deleted. `verify-release-export.ps1`'s OK line said "여섯 로케일 블록" through two additions while the assertion under it was right all along. And the prose counts in `AGENTS.md`, `docs/RELEASE.md` and `docs/assets/README.md` are now checked against the list, because a count is the one thing that cannot be derived from it.
- **The tests were not re-running.** `config/locales.tsv` was not a declared input of the unit test tasks, so editing it alone left `testDebugUnitTest` UP-TO-DATE and the previous verdict standing. CI checks out clean and would never have seen it; the person adding a language would have. Declared as an input, after which reinstating the drift failed all four locale tests.
- Codex caught two real holes in the new gate on the first commit, both fixed before merge. `values-en` would have passed, because the source language *is* declared — Android prefers it on English devices while `ResourceParityTest` keeps reading `values/`; the suffixed surfaces now compare against the translated codes only. And two rows could have shared a `store` tag, satisfying both against one fastlane directory while the notes file carried the block twice.
- **The formatting-row golden needed a seam, not just a test** (PR #336). `EditorScreen` built its own `AppSettingsRepository`, and a Robolectric test cannot put that into a chosen state — the `preferencesDataStore` delegate caches one instance per JVM and DataStore's `File.renameTo` writes fail on Windows after the first, which is the wall #158 already hit and why the repository has an internal DataStore constructor. The screen now takes the repository as a defaulted parameter, so the seam that makes the repository testable makes the screen it feeds testable too. `InMemoryPreferencesDataStore` moved out of `AppSettingsRepositoryLockTest`, where it was a private copy also duplicated in the list test.
- Why it had to be the screen: what "off" produces is an absence with nothing left behind. The instrumented assertion says the node does not exist, and `EditorFormattingControlsSnapshotTest` photographs a row that in this state is not mounted at all — a layout change that kept the row's 48dp while dropping its content, which is the complaint #328 filed, passes both. The new golden also asserts its own premise before capturing: an image cannot say why it looks the way it does, and one defined by an absence would look plausible if the setting had not reached the composition yet.
- Verification: the gate was broken in every direction it claims to cover — the `hr` row dropped (12 failures naming every `hr` file), a language declared with no files, `docs/privacy.hr.html` removed, `values-en` and `index.en.html` added, a duplicate `store` tag. `./gradlew test` (all variants) green, the three PR-time verify scripts green, and `verify-release-export.ps1` green against the real v2.33.0 artifacts in `D:\Build`, now printing the count and locales it checked. The Gradle snippet was executed against the manifest to confirm it yields the same eight store tags in the same order as the list it replaces. The golden was recorded on the Linux runner via `record_roborazzi`; **one new image, the other 69 byte-identical**, so the new parameter moved no rendering. `build`, `instrumented-tests` and `launch-smoke` green on both PRs.
- No `CHANGELOG.md` entry: nothing here is user-visible. The seam is a defaulted parameter over the same store, and the byte-identical goldens are the evidence.
- What is left in #262 after this: RTL cursor and selection on a device, RTL UI strings (`values-ar` / `values-he` / `values-fa`), the editor's jump direction — costed on 2026-08-14 and declined with the two migration routes priced — and the sidecar index rewrite, which needs an on-disk format change and a decision record. All four are deferred on purpose, not overlooked.

## 2026-08-23 - v2.33.0 (versionCode 129)

- Trigger: a maintainer pass over one external PR and one Discussion thread, both of which turned out to be worth taking. #329 was a Croatian translation; #328 was a request to stop the formatting button sitting above the keyboard. Neither is a bug, and neither would have shipped on its own — together they are a minor.
- **The contribution was more complete than the repository was ready for.** #329 arrived with 322 keys at 1:1 parity, every format specifier matching, Croatian's `few` plural filled in on all nine `<plurals>`, plus `raw-hr/starter_notes.md` and both Play descriptions — more than #294 (Chinese) brought. What it could not bring is the part that is not visible from the files it touched: the locale list is hardcoded in seven places, and three of them are release plumbing. `raw-hr` existed but `localizedStarterNotesExist` did not look at it; `noteLocales` and the two `$StoreLocales` decide whether the export task and the release gates see the locale at all. A locale missing from those ships green and untested, which is why this was filled in by hand rather than sent back to the contributor.
- Two defects in the contribution, both found rather than assumed. `short_description.txt` was 83 characters against Play's 80 — nothing in the repository checked that, because those two files do not change per release, so nobody re-counts them; a new locale counts them for the first time. `verify-release-notes.ps1` now checks both descriptions against Play's limits for every locale, which is a gate the release process should have had before the first translation arrived.
- **Three Croatian slips came from running it, not reading it.** The debug build on an API 36 emulator in `hr-HR` showed "Ovaj bilješka" (*bilješka* is feminine), "pikazuje"/"umjeto" for *prikazuje*/*umjesto*, and `onboarding_body_welcome` in the accusative where de/es/fr are all nominative. A key-parity diff sees none of those. All three were fixed and put back to the contributor in the PR, since a non-native fix is a proposal, not a correction.
- Public surfaces followed the same rule the other seven languages do: `README.hr.md`, `docs/index.hr.html`, `docs/privacy.hr.html`, the switcher in all eight of each, and `markleaf-tablet-hr` in mp4 / gif / poster. **The demo clip was cropped, not re-shot** — `crop=1920:934:0:0` on the English master strips the subtitle band and leaves a clean base, and re-recording would have shifted the clock, the timing and the app version out of step with its seven siblings. 960x540, 238 frames, same spec as the rest.
- #331 (from #328) is the smaller half and the one with a design line in it. Off removes the *standing* `Aa` entry, not formatting: the bold / italic / link row that appears while text is selected stays, because selecting is how you ask for it, and a gate that dropped both would leave no touch path to formatting and still look like it worked. Implemented by not mounting the controls rather than hiding the button inside them — hiding leaves the row's 48dp above the keyboard, which is the complaint. The compact-toolbar half of the request was declined in the thread with the reason attached: a permanent row of format buttons is more chrome, not less.
- Roborazzi: exactly one golden moved (`settings_centered_tablet`, the Markdown section gains a row). Recorded on the Linux runner via `record_roborazzi`; the other 59 came back byte-identical, checked by diffing the whole artifact rather than trusting the count. Codex flagged the stale golden on the first commit, which was already fixed on the head it had not read — and separately caught a real one on #330: `docs/RELEASE.md`'s post-tag recovery loop still enumerated seven locales, so a recovered archive would have failed `verify-release-export.ps1` with the tag already public.
- Release mechanics: versionCode 128→129, versionName 2.32.6→2.33.0, CHANGELOG both editions, **eight** store-locale `129.txt` (429 characters at the longest), landing ×8 (three version strings plus the release date) and README ×8. First cut with eight store locales.
- Verification: `verify-release-notes.ps1`, `verify-landing-versions.ps1`, `check-landing-overflow.mjs` (16 pages × 6 widths) and `verify-release-export.ps1` all green; `build` and `instrumented-tests` green on all three PRs. `exportReleaseToBuildDrive` ran locally this time — the AAB (5.6 MB), the 42.9 MB mapping and the eight-locale notes are in `D:\Build`, and the AAB carries `CN=Markleaf, O=Markleaf, L=Seoul, C=KR`. The published APK's V2 signer is `0be97352…f91a`, the same key existing installs and F-Droid expect.
- Hardening candidates went to #262 as a `## v2.33.0` section: the seven-place locale list with nothing tying it together, `.gitlab-ci.yml` still asserting six locale blocks behind a switch nobody turns on, no check that a landing language has a privacy page, `verify-release-export.ps1` reporting "여섯 로케일 블록" whatever the count, and no golden for the editor with the formatting row gone. No new issue opened — the release flow spends the tracker rather than refilling it.

## 2026-08-15 - v2.32.6 (versionCode 128)

- Trigger: #318 merged, and it is the first user-visible change since v2.32.5. Four commits sit between the two releases and three of them are docs or gitignore (#316, #317, #320), so the cut is one line long. A one-fix release is worth cutting anyway when the fix is the app's icon: it is the only part of the app every user sees before they open it.
- What ships: the launcher icon no longer overflowing the adaptive-icon safe zone. Nothing else earns a line. #320 credits the contributor and #317 ignores JVM crash dumps; neither changes what anyone sees.
- Review, not rubber-stamping. The PR arrived with three parts and only one of them was the fix. `applicationIdSuffix = ".debug"` was reverted out of it — `.github/scripts/launch-smoke.sh` hardcodes `com.markleaf.notes` in both its `pm path` poll and its `am start` component, so the suffix would have made that job fail on every run, and a non-required check that always fails is worse than no check. Tracked as #319 with the script fix attached. The suffix is a good idea; it just cannot land alone.
...
## 2026-09-17 — v2.46.0: find in note from preview (#417)

`ray4423` asked for Find in note in preview, where the overflow menu had hidden it. Preview find matches the text the preview draws (`PreviewFind.kt` lists each row's drawn pieces in render order), not the Markdown source, so bullets, callout labels and link addresses never match while bold words and link labels do. A match in a collapsed `<details>` section is counted over the full row list and expands its section when reached; replace stays in edit mode because it edits the source. Codex review ran six rounds on PR #421 and every finding was fixed in the PR (the last commit went unreviewed when Codex hit its usage limit, so a separate agent reviewed it and found one more defect — a match reveal that fired whenever its row recomposed, pulling the view back while find stayed open; reveals are now one-shot per step, with a regression test that fails without the fix): quadratic table offset counting, missing-attachment fallback text not searched, attachments resolved on every preview build even with find closed, a stale query acting for one frame after an edit/preview switch, results left under the IME, and matches deep inside one tall row (every occurrence maps to the same LazyColumn item) left off screen — now each text piece brings its current match into view with a `BringIntoViewRequester` after the row is on screen. The mode-switch and tall-row regression tests were each confirmed to fail with their fix removed. Verified on the `markleaf-phone-api36` emulator: highlights, `1/6` → `6/6` expanding a collapsed section, and the last line of a 40-paragraph note staying above the keyboard. The S24 was reachable but held an install this session did not make, so it was left untouched. `EditorMovingBlankNoteToTrashStaysRecoverableTest` failed once locally with a Robolectric looper error from a popup and passed on two isolated reruns; recorded here as a flake, unrelated to this change.
---

## 2026-09-18 — v2.46.1: edit-mode find offsets after a length-changing lowercase (#262)

Filed in #262 while releasing v2.46.0 and fixed on request the same night. `findAllRanges` searched `text.lowercase()` and returned those offsets as ranges into the original text, but lowercasing is not length-preserving: `İ` (U+0130) lowercases to `i` plus a combining dot. Each such character before a match shifted the selection and the replace target one position late, and a match near the end produced a range past the text. Three new `FindRangesTest` cases failed first on the old code — including `İzmir` searched for `i`, where the old code reported index 4 (the `r`) instead of 3 — and pass now that `findAllRanges` maps `findOccurrences` (`indexOf(ignoreCase = true)`, the preview's comparison) onto ranges. Verified on the `markleaf-phone-api36` emulator: in `İstanbul İzmir apple end`, find `apple` selected exactly `apple` and Replace all produced `İstanbul İzmir PEAR end`.
