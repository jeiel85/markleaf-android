...
- **APK の直接インストール**: [GitHub v2.53.1](https://github.com/jeiel85/markleaf-android/releases/tag/v2.53.1) リリースには APK が 2 つあります — `markleaf-v2.53.1.apk` は F-Droid/Play と同じビルド(自動更新なし、追加の権限なし)、`markleaf-v2.53.1-sideload.apk` はアプリ内での更新確認機能付きのビルドです(`INTERNET`、`REQUEST_INSTALL_PACKAGES` の権限が加わります)。アプリ内更新を使いたい場合は sideload 版をダウンロードして Android 端末で実行してインストールしてください — どちらも同じ署名鍵を使うため、後で入れ替えても再インストールではなく通常の更新として扱われます。
