...
## 2026-07–10 — 안정성, 배포, 사용자 증거 (현재 우선순위)

기준일: 2026-07-28, v2.32.1

v2.23 이후 Quiet Editor, 온보딩, 잠금, 동기화 신뢰성, 노트 탐색과 공개 표면을 계속
다듬었다. 이 시점의 병목은 기능 수가 아니라 **신뢰할 수 있는 배포 경로와 반복 사용자의
증거**다. 따라서 아래 기간에는 새 대형 기능을 기본값으로 추가하지 않는다.

제품 계약:

- Markleaf는 **평면 폴더에서 쓰는 Android 로컬 우선 Markdown 메모 앱**이다. 중첩 폴더와
  파일명 기반 링크가 많은 기존 vault의 완전한 대체재로 약속하지 않는다.
- 최신 signed APK의 기준점은 GitHub Releases다. F-Droid는 자동 업데이트에 권장하지만
  카탈로그 반영은 GitHub보다 늦을 수 있다. GitLab은 소스 미러이며, 릴리스 자산의 가용성을
  사용자 설치 경로로 약속하지 않는다. Google Play 업데이트는 정책 문제가 해결될 때까지 보류한다.
- 새 기능은 실제 사용자 피드백, 데이터 안전성, Android 호환성 문제 중 하나를 직접 해결할 때만
  이 기간에 들어온다.

작업:

- GitHub, F-Droid, Google Play, GitLab의 역할과 최신 버전 안내를 README 6개 언어에서 같은
  문구로 유지한다.
- GitLab 릴리스 자산 게시 실패를 복구하거나, 복구 전까지 그 채널을 설치 안내에서 제외한다.
- 서로 다른 폴더 동기화 도구와 실제 Markdown 폴더를 쓰는 테스터 5–10명에게 편집·동기화·복구
  흐름을 검증받는다.
- 공개 Discussions/이슈에서 설치 경로, 앱 버전, 재현 단계만 받는 가벼운 피드백 흐름을 유지한다.
  앱 내 분석·추적·계정은 추가하지 않는다.
- 배포와 핵심 테스트가 안정될 때까지 정식 릴리스를 2–4주 단위로 묶고, 긴급 데이터 손실 수정만
  예외로 한다.

판단 게이트:

- 3개월 안에 서로 다른 사용자에게서 재현 가능한 피드백 10건 이상과 반복 사용자 신호가 쌓이면,
  가장 큰 동기화·온보딩 문제를 다음 한 사이클의 후보로 고른다.
- 6개월 동안 피드백과 설치 경로의 성장 증거가 없거나 유지보수 부담이 주당 약 4시간을 넘으면,
  기능 개발을 멈추고 Android 호환성·보안·데이터 안전성만 다루는 유지보수 모드로 전환한다.
...
아래 Phase 29–33은 당시의 기능 제안이다. 버전 번호나 구현 약속으로 해석하지 않으며,
위 안정성·배포·사용자 증거 게이트를 통과한 뒤에만 다시 우선순위를 매긴다.
