...
- **산출물 분리.** productFlavor `store`(F-Droid·Play·재현 빌드 대상)와 `github`(사이드로드
  전용)로 나눈다. `android.permission.INTERNET`과 업데이터 코드는 `github` 플레이버의 소스
  세트에만 존재한다. `store` 빌드의 권한 목록은 이 절이 도입되기 전과 동일하다.
...
- **F-Droid 재현 빌드 보호.** `Binaries:` + `AllowedAPKSigningKeys` 검증 대상은 `store`
  산출물이다. 업스트림 fdroiddata 레시피가 `store` 변형을 빌드하도록 고쳐지기 전에는 이
  플레이버 구조를 담은 릴리스 태그를 밀지 않는다.
